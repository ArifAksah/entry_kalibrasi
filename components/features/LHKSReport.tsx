...
const LHKSReport: React.FC<LHKSReportProps> = ({
  ...
  instrumentNames,
  dataLoading = false,
  loadingMessage = 'Menyiapkan data LHKS…',
}) => {
  ...
  if (dataLoading) {
    return <LoadingOverlay label={loadingMessage} />
  }
  ...
  const effectiveCalibrationDate =
    calibrationDate || resultSetup?.start_date || certificate.issue_date

  // Fallback calibration location from results
  const effectiveCalibrationLocation =
    calibrationLocation || resultDisplay?.place || ''

  // Fallback environment conditions from results
  const effectiveEnvironmentConditions =
    environmentConditions ||
    (() => {
  ...
  const cleanStdValue = (r: any): number | null => {
    if (
      typeof r?.std_corrected === 'number' &&
      Number.isFinite(r.std_corrected)
    )
      return r.std_corrected
    if (
      typeof r?.standard_data === 'number' &&
      Number.isFinite(r.standard_data)
    ) {
      return (
        r.standard_data +
        (typeof r?.std_correction === 'number' ? r.std_correction : 0)
      )
    }
    return null
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[70] backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="px-4 py-2 border-b border-gray-100 bg-white flex items-center gap-4 text-xs text-gray-600 flex-wrap">
          ...
          {Object.keys(qcLimits).length === 0 && rawData.length > 0 && (
            <span className="text-yellow-600">
              ⚠ Memuat batas QC dari Master QC...
            </span>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto p-8 bg-gray-100">
          <div
            ref={printRef}
            className="bg-white p-[10mm] shadow-lg max-w-[210mm] mx-auto min-h-[297mm] text-black font-serif text-[9.5pt] leading-snug"
          >
            <style>{`
                            @media print {
            ...
            <div className="no-break">
              ...
              <h3 className="text-center font-bold text-[11pt] mb-4 underline">
                LAPORAN HASIL KALIBRASI SEMENTARA
              </h3>

              <div className="mb-3">
                <p>
                  <span className="font-bold w-28 inline-block">No. Order</span>
                  <span>: {certificate.no_order}</span>
                </p>
              </div>

              {/* IDENTITAS ALAT */}
              <div className="mb-5">
                <h4 className="font-bold underline mb-1">
                  IDENTITAS ALAT /{' '}
                ...
                <table className="w-full border-none text-left">
                  <tbody>
                    <tr>
                      ...
                      >
                        Nomor Kalibrasi /{' '}
                        <span className="italic">Calibration Number</span>
                      </td>
                    ...
                    <tr>
                      <td className="border-none align-top">
                        Nama / <span className="italic">Name</span>
                      </td>
                      ...
                      <td className="border-none align-top">
                        {(() => {
                          ...
                          if (
                            effectiveInstrument?.name &&
                            !/^\d+$/.test(
                              String(effectiveInstrument.name).trim(),
                            )
                          ) {
                            return effectiveInstrument.name
                          }
                          // Try to get name from instrumentNames lookup
                          const instrumentNameObj = instrumentNames?.find(
                            (n) =>
                              n.id ===
                              (effectiveInstrument as any)?.instrument_names_id,
                          )
                          if (
                            instrumentNameObj?.name &&
                            !/^\d+$/.test(String(instrumentNameObj.name).trim())
                          ) {
                            return instrumentNameObj.name
                          }
                          // Fallback to first non-standard sensor name
                          const firstSensor = effectiveSensors.find(
                            (s: Sensor) => !s.is_standard,
                          )
                          if (firstSensor) {
                            const sensorName = getSensorDisplayName(firstSensor)
                            if (
                              sensorName &&
                              !sensorName.startsWith('Sensor #')
                            ) {
                              return sensorName
                            }
                          }
                    ...
                    <tr>
                      ...
                      <td className="border-none align-top">:</td>
                      <td className="border-none align-top">
                        {effectiveInstrument?.manufacturer || '-'}
                      </td>
                    </tr>
                    <tr>
                      <td className="border-none align-top">
                        Tipe / <span className="italic">Type</span>
                      </td>
                      <td className="border-none align-top">:</td>
                      <td className="border-none align-top">
                        {effectiveInstrument?.type || '-'}
                      </td>
                    </tr>
                    <tr>
                      <td className="border-none align-top">
                        Nomor Seri /{' '}
                        <span className="italic">Serial Number</span>
                      </td>
                      <td className="border-none align-top">:</td>
                      <td className="border-none align-top">
                        {effectiveInstrument?.serial_number || '-'}
                      </td>
                    </tr>
                    <tr>
                      <td className="border-none align-top pt-1">
                        Resolusi / <span className="italic">Resolution</span>
                      </td>
              ...
              <div className="mb-5">
                ...
                <table className="w-full border-none text-left">
                  <tbody>
                    ...
                    <tr>
                      <td className="border-none align-top">
                        Alamat / <span className="italic">Address</span>
                      </td>
                      <td className="border-none align-top">:</td>
                      <td className="border-none align-top">
                        {owner?.address || (
                ...
                <table className="w-full border-none text-left">
                  <tbody>
                    ...
                    <tr>
                      ...
                      >
                        Rentang Kalibrasi /{' '}
                        <span className="italic">calibration range</span>
                      </td>
                      <td className="border-none align-top w-2"></td>
                      <td className="border-none align-top"></td>
                    </tr>
                    ...
                    {effectiveSensors
                      ...
                      .map((s: Sensor) => {
                        ...
                        const sensorRows = rawData.filter(
                          (r) => r.sensor_id_uut === s.id,
                        )
                        ...
                        if (stdCorrectedVals.length > 0) {
                          ...
                          const rawRange = s.range_capacity
                            ? String(s.range_capacity).trim()
                            : ''
                          const rawUnit = s.range_capacity_unit
                            ? String(s.range_capacity_unit).trim()
                            : ''
                    ...
                    {/* Spacer */}
                    <tr>
                      <td colSpan={3} className="border-none py-1"></td>
                    </tr>
                    ...
                    {/* Spacer */}
                    <tr>
                      <td colSpan={3} className="border-none py-0.5"></td>
                    </tr>

                    {/* Tempat / Place */}
                    <tr>
                      <td className="border-none align-top">
                        Tempat / <span className="italic">Place</span>
                      </td>
                      <td className="border-none align-top">:</td>
                      <td className="border-none align-top">
                        {effectiveCalibrationLocation || owner?.name || '-'}
                      </td>
                    </tr>
                    ...
                    {/* Kondisi Ruang */}
                    <tr>
                      <td className="border-none align-top text-blue-700 italic">
                        Kondisi Ruang /{' '}
                        <span className="italic">room condition</span>
                      </td>
              ...
              {/* Page 1 page number */}
              <div className="text-right text-xs text-red-600 mt-4">
                Halaman 1 dari 2
              </div>
            </div>
            ...
            <div className="no-break mb-6">
                  ...
                  <tbody>
                    ...
                    {effectiveSensors
                      ...
                      .map((s: Sensor) => {
                        ...
                        const stdSensorId = rawData.find(
                          (r) => r.sensor_id_uut === s.id,
                        )?.sensor_id_std

                        const stdSensor = allSensors?.find(
                          (x) => x.id === stdSensorId,
                        )
                        const stdInstrument = allInstruments?.find(
                          (x) => x.id === stdSensor?.instrument_id,
                        )
                        const matchedStdCert =
                          standardCerts.find(
                            (sc) => sc.sensor_id === stdSensorId,
                          ) ?? standardCerts[0]
                        ...
                        if (stdSensor || stdInstrument || matchedStdCert) {
                          ...
                          if (!stdName || /^\d+$/.test(stdName)) {
                            stdName = instrumentNames?.find(
                              (n) => n.id === stdSensor?.sensor_name_id,
                            )?.name
                          }
                          if (!stdName || /^\d+$/.test(stdName)) {
                          ...
                          const manufacturer =
                            stdSensor?.manufacturer ||
                            stdInstrument?.manufacturer
                          const type = stdSensor?.type || stdInstrument?.type
                          const serial =
                            stdSensor?.serial_number ||
                            stdInstrument?.serial_number
                    ...
                    {standardCerts.length === 0 && (
                      <tr>
                        <td
                          colSpan={3}
                          className="border-none italic text-gray-400 pl-6"
                        >
                          Tidak ada data standar tersedia
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div className="mb-5">
                <h4 className="font-bold underline mb-1">
                  KETELUSURAN /{' '}
                  <span className="italic font-normal">Traceability</span>
                </h4>
                <table className="w-full border-none text-left">
                  <tbody>
                    {effectiveSensors
                      ...
                      .map((s: Sensor) => {
                        ...
                        const matchedStd = standardCerts.find(
                          (sc) => (sc as any).sensor_id === s.id,
                        )
                        const traceOrg =
                          (matchedStd as any)?.traceable_to ||
            ...
            {Object.keys(groupedData).map((sensorKey, pageIdx) => {
              ...
              const sensorName = sensor
                ...
                : data[0]?.sheet_name || `Sensor #${sensorKey}`
              const canonicalSensorName = sensor?.sensor_name_id
                ? instrumentNames?.find(
                    (name: any) => name.id === sensor.sensor_name_id,
                  )?.name
                : null
              const sessionResult = Array.isArray(sessionResults)
                ? sessionResults.find(
                    (result: any) =>
                      String(result.sensorId ?? result.sensor_id) === sensorKey,
                  )
                : null
              ...
              const qcLimit =
                sensorKey !== 'unknown' ? (qcLimits[sensorKey] ?? null) : null
              ...
              const hasUnitMismatch =
                rowUnitStd &&
                rowUnitUut &&
                needsConversion(rowUnitStd, rowUnitUut)
              ...
              const getStdConverted = (row: (typeof data)[0]) => {
                const stdCorr =
                  row.std_corrected ??
                  (row.standard_data ?? 0) + (row.std_correction ?? 0)
                if (!hasUnitMismatch) return stdCorr
                return convertUnit(
                  stdCorr,
                  row.unit_std || rowUnitStd,
                  row.unit_uut || rowUnitUut,
                )
              }

              const getUutCorrection = (row: (typeof data)[0]) => {
                if (
                  row.uut_correction != null &&
                  Number.isFinite(row.uut_correction)
                ) {
              ...
              const avgStdCorrected =
                data.reduce((sum, row) => sum + getStdConverted(row), 0) /
                (totalRows || 1)
              const avgUutData =
                data.reduce((sum, row) => sum + row.uut_data, 0) /
                (totalRows || 1)
              ...
              if (isPyranoForAvg) {
                ...
                avgCf =
                  cfValues.length > 0
                    ? cfValues.reduce((sum, cf) => sum + cf, 0) /
                      cfValues.length
                    : 0
                ...
                const varianceCf =
                  cfValues.length > 1
                    ? cfValues.reduce(
                        (sum, cf) => sum + Math.pow(cf - avgCf, 2),
                        0,
                      ) /
                      (cfValues.length - 1)
                    : 0
                ...
                const stdValues = data
                  .filter((r) => r.standard_data > 0)
                  .map((r) => r.standard_data)
                const avgStd =
                  stdValues.reduce((a, b) => a + b, 0) / (stdValues.length || 1)
                const varianceStd =
                  stdValues.length > 1
                    ? stdValues.reduce(
                        (a, b) => a + Math.pow(b - avgStd, 2),
                        0,
                      ) /
                      (stdValues.length - 1)
                    : 0
                ...
                // Statistik UUT
                const uutValues = data
                  .filter((r) => r.uut_data > 0)
                  .map((r) => r.uut_data)
                const avgUut =
                  uutValues.reduce((a, b) => a + b, 0) / (uutValues.length || 1)
                const varianceUut =
                  uutValues.length > 1
                    ? uutValues.reduce(
                        (a, b) => a + Math.pow(b - avgUut, 2),
                        0,
                      ) /
                      (uutValues.length - 1)
                    : 0
                ...
                avgCorrection =
                  corrections.length > 0
                    ? corrections.reduce((sum, c) => sum + c, 0) /
                      corrections.length
                    : 0
                ...
                const variance =
                  corrections.length > 1
                    ? corrections.reduce(
                        (sum, c) => sum + Math.pow(c - avgCorrection, 2),
                        0,
                      ) /
                      (corrections.length - 1)
                    : 0
              ...
              const matchedStdCert =
                standardCerts.find((sc) => sc.sensor_id === stdSensorId) ??
                standardCerts[0]
              ...
              if (stdSensor || stdInstrument || matchedStdCert) {
                ...
                if (!stdName || /^\d+$/.test(stdName))
                  stdName = stdInstrument?.name || undefined
                if (!stdName || /^\d+$/.test(stdName))
                  stdName = (matchedStdCert as any)?.sensor_name
                stdNameStr = stdName || '-'
              }
              const stdMerkTipeSN =
                [
              ...
              if (stdCorrectedVals.length > 0) {
                ...
                const rawRange = sensor?.range_capacity
                  ? String(sensor.range_capacity).trim()
                  : ''
                const rawUnit = sensor?.range_capacity_unit
                  ? String(sensor.range_capacity_unit).trim()
                  : ''
                const isRangeValid = rawRange && !/^\d+$/.test(rawRange)
                rangeDisplay = isRangeValid
                  ? `${rawRange} ${rawUnit}`.trim()
                  : qcLimit
                    ? `± ${qcLimit.rawLimit} ${qcLimit.unit}`
                    : '-'
              }

              // Environment details
              const globalTemp =
                effectiveEnvironmentConditions?.temperature || '-'
              const globalHum = effectiveEnvironmentConditions?.humidity || '-'
              const sessionRes = Array.isArray(sessionResults)
                ? sessionResults.find(
                    (r: any) =>
                      r.sensorId === (sensor?.id ?? Number(sensorKey)) ||
                      r.sensor_id === (sensor?.id ?? Number(sensorKey)),
                  )
                : null
              ...
              if (data.length > 0) {
                ...
                if (d.getTime() > 0 && d.getFullYear() > 1970) {
                  targetDateStr = d.toLocaleDateString('id-ID', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                  })
              ...
              return (
                <div
                  key={sensorKey}
                  className={`no-break mb-8 w-full ${pageIdx > 0 ? 'page-break' : ''}`}
                >
                  <div className="text-center font-bold mb-3 text-sm">
                    Tabel {(pageIdx + 2).toString().padStart(2, '0')}. Data
                    Mentah Hasil Kalibrasi {sensorName.toUpperCase()}
                  </div>
                      ...
                      <tr>
                        <td className="w-1/2 p-0 align-top border-r border-black">
                          ...
                          <table className="w-full border-none border-collapse text-left">
                            <tbody>
                              <tr>
                                <td className="border-none w-32 px-1">
                                  No. Order/Identifikasi
                                </td>
                              ...
                              <tr>
                                <td
                                  colSpan={2}
                                  className="border-t border-b border-black text-center relative underline pt-1"
                                >
                                  {' '}
                                  Kondisi Ruangan
                                </td>
                                ...
                                <td colSpan={2} className="p-0 border-none">
                                  ...
                                  <table className="w-full text-left border-none border-collapse bg-transparent">
                                    <tbody>
                                      ...
                                      <tr className="border-t border-black">
                                        <td className="border-none border-r border-black text-left px-1">
                                          Temperatur
                                        </td>
                                        <td className="border-none px-1">
                                          {' '}
                          ...
                          <table className="w-full border-none border-collapse text-left">
                            <tbody>
                              <tr>
                                <td
                                  colSpan={2}
                                  className="px-1 border-b border-black underline"
                                >
                                  Alat Standar
                                </td>
                              </tr>
                              <tr>
                                <td className="border-none w-28 px-1">
                                  Nama Alat
                                </td>
                                <td className="border-none px-1">
                                  : {stdNameStr}
                  ...
                  <table className="w-full text-xs border-collapse border border-black text-center">
                    <thead>
                      <tr>
                        <th
                          ...
                          className="bg-gray-100 border border-black font-bold text-center uppercase"
                        >
                          DATA HASIL KALIBRASI {sensorName}
                        </th>
                      </tr>
                      {isPyranometer(
                        sensor
                        ...
                        <>
                          <tr>
                            ...
                            <th className="border border-black bg-green-100">
                              Faktor Kalibrasi
                        ...
                        <>
                          <tr>
                            ...
                            <th
                              rowSpan={2}
                              className="border border-black leading-tight"
                            >
                              Alat yang
                              <br />
                              dikalibrasi
                    ...
                    <tbody>
                      {sampled.map((row, idx) => {
                        ...
                        const stdCorrectionStr =
                          stdCorrectionRaw === 0
                            ? '0'
                            : stdCorrectionRaw.toFixed(4).replace(/\.?0+$/, '')
                        const stdCorrected =
                          row.std_corrected ??
                          row.standard_data + stdCorrectionRaw
                        ...
                        const cfValue =
                          isPyrano && row.standard_data > 0 && row.uut_data > 0
                        ...
                        if (
                          ...
                          rawCorrection =
                            (row.standard_data / row.uut_data - 1) * 100
                        } else {
                          // BIASA: koreksi = Std - UUT
                          ...
                          <React.Fragment key={`${row.id}-${idx}`}>
                            {data.length > 30 && idx === 15 && (
                              <tr>
                                <td
                                  colSpan={
                                    isPyrano ? 4 : hasUnitMismatch ? 7 : 6
                                  }
                                  className="border border-black text-center italic py-1 bg-gray-50 text-gray-500"
                                >
                                  {' '}
                            ...
                            {isPyrano ? (
                              ...
                              <tr
                                className={isFail ? 'bg-pink-100' : ''}
                                style={
                                  isFail ? { backgroundColor: '#fce7f3' } : {}
                                }
                              >
                                <td className="border border-black px-1">
                                  {displayIdx}
                                ...
                                <td className="border border-black px-1">
                                  ...
                                  {stdCorrectionRaw !== 0 && (
                                    <span className="print:hidden ml-1">
                                      <SigFigBadge value={stdCorrectionRaw} />
                                    </span>
                                  )}
                                </td>
                                <td className="border border-black px-1">
                                ...
                                {hasUnitMismatch && (
                                  <td className="border border-black px-1 bg-blue-50/40">
                                    {stdConverted.toFixed(4)}
                                    <span
                                      className="text-[8px] text-gray-400 ml-0.5"
                                      title={`Dikonversi dari ${row.unit_std || rowUnitStd} ke ${row.unit_uut || rowUnitUut}`}
                                    >
                                      *
                                    </span>
                                    <span className="print:hidden ml-1">
                                      <SigFigBadge value={stdConverted} />
                                    </span>
                                  </td>
                                )}
                                <td className="border border-black px-1">
                                ...
                                <td
                                  className={`border border-black px-1 ${isFail ? 'text-red-600 font-bold' : ''}`}
                                >
                                  {rawCorrection
                                    .toFixed(6)
                                    .replace(/\.?0+$/, '') || '0'}
                                  <span className="print:hidden ml-1">
                                    <SigFigBadge value={rawCorrection} />
                                  </span>
                                </td>
                              </tr>
                            )}
                      ...
                      {isPyranoForAvg ? (
                        ...
                        <>
                          <tr>
                            <td
                              colSpan={hasUnitMismatch ? 4 : 3}
                              className="border border-black text-left font-bold px-1 pl-2"
                            >
                              Rata-Rata
                            </td>
                            <td className="border border-black font-bold px-1">
                              ...
                              <span className="print:hidden ml-1">
                                <SigFigBadge value={avgStdCorrected} />
                              </span>
                            </td>
                            <td className="border border-black font-bold px-1">
                              {' '}
                              {avgUutData.toFixed(2)}
                              <span className="print:hidden ml-1">
                                <SigFigBadge value={avgUutData} />
                              </span>
                            </td>
                            <td className="border border-black font-bold px-1">
                              {' '}
                              {avgCorrection.toFixed(6).replace(/\.?0+$/, '') ||
                                '0'}
                              <span className="print:hidden ml-1">
                                <SigFigBadge value={avgCorrection} />
                              </span>
                            </td>
                          </tr>
                          <tr>
                            <td
                              colSpan={hasUnitMismatch ? 6 : 5}
                              className="border border-black text-left font-bold px-1 pl-2"
                            >
                              Standar Deviasi
                            </td>
                            <td className="border border-black px-1">
                              ...
                              {stdDevCorrection
                                ...
                                .replace(/\.?0+$/, '') || '0'}
                              <span className="print:hidden ml-1">
                                <SigFigBadge value={stdDevCorrection} />
                              </span>
                            </td>
                          </tr>
                        </>
                  ...
                  {hasUnitMismatch && (
                    <div className="mt-1 text-[10px] text-gray-500 italic text-right">
                      * Kolom STD Pembacaan, Koreksi, dan Terkoreksi tetap dalam{' '}
                      <strong>{formatUnit(rowUnitStd)}</strong>. Kolom
                      Terkoreksi Hasil Konversi dan Koreksi akhir menggunakan{' '}
                      <strong>{formatUnit(rowUnitUut)}</strong>.
                    </div>
                  )}
                </div>
              )
