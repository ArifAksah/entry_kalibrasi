...
function UncertaintyContent({
  ...
  if (rawStandardCertificateId) {
    standardCertRecord =
      standardCerts.find(
        (c) => Number(c.id) === Number(rawStandardCertificateId),
      ) ?? null
  }
  if (
    !standardCertRecord &&
    ...
    if (m.standardCertificateId) {
      standardCertRecord =
        standardCerts.find((c) => c.id === m.standardCertificateId) ?? null
    }
  }
  ...
  if (!standardCertRecord && stdSensorId) {
    ...
    if (certs.length > 0) {
      standardCertRecord = certs.sort(
        (a, b) =>
          new Date(b.calibration_date).getTime() -
          new Date(a.calibration_date).getTime(),
      )[0]
    }
  }
  ...
  const rawResolusiUut =
    uutSensor?.resolution != null
  ...
  if (!unitUut && certificate.instrument) {
    ...
    if (certInst?.sensor && certInst.sensor.length > 0) {
      unitUut =
        certInst.sensor[0].graduating_unit ||
        certInst.sensor[0].range_capacity_unit
    }
  }
  unitUut = unitUut || 'Unit'

  const driftStd =
    typeof standardCertRecord?.drift === 'number'
  ...
  const uutInstrument = certificate.instrument
    ? instruments.find((i) => i.id === certificate.instrument)
    : null
  ...
  const uutReadings = React.useMemo(() => {
    return currentData.map((row) => {
      ...
      const stdCorrectedInUutUnit =
        unitStdRow &&
        unitUutRow &&
        unitStdRow.toLowerCase() !== unitUutRow.toLowerCase()
  ...
  const sensorNameDisplay = React.useMemo(() => {
    ...
    const fromLookup = uutSensor.sensor_name_id
      ? instrumentNames?.find((n: any) => n.id === uutSensor.sensor_name_id)
          ?.name
      : undefined
    if (fromLookup) return fromLookup.toUpperCase()
    if (uutSensor.name && !/^\d+$/.test(String(uutSensor.name).trim()))
      return uutSensor.name.toString().toUpperCase()
  ...
  if (stdCorrectionPoints.length > 0) {
    ...
    interpolatedU95 = interpolateU95FromPoints(
      stdCorrectionPoints,
      globalStdCorrected,
    )
    ...
    interpolatedU95 =
      typeof standardCertRecord.u95_general === 'number'
  ...
  if (isPyranometerSensor) {
    ...
    const uutMeanVal =
      uutReadingsForCF.length > 0
        ? uutReadingsForCF.reduce((a, b) => a + b, 0) / uutReadingsForCF.length
        : 0
    ...
    components = pyranometerResult.components.map((c) => ({
      ...
      unit: '%',
      distribution:
        c.distribution === 'Normal' ? ('Normal' as const) : ('Rect' as const),
      symbol:
        c.name === 'Repeat'
          ? 'u_rep'
          : c.name === 'Sertifikat Std'
            ? 'u_sertf'
            : c.name === 'Resolusi Std'
              ? 'u_res_std'
              : c.name === 'Drift Std'
      ...
      ci_ui_quad_vi:
        Math.pow(c.u_percent, 4) /
        (c.deg_freedom === Infinity ? 1e9 : c.deg_freedom),
    }))

    // Hitung effective degrees of freedom (Welch-Satterthwaite)
    const sum_ci_ui_quad_vi = components.reduce(
      (a, c) => a + c.ci_ui_quad_vi,
      0,
    )
    const eff_deg_freedom =
      sum_ci_ui_quad_vi > 0
  ...
  components = components.map((c) => {
    ...
    if (c.name.includes('Cert Std U95'))
      return { ...c, name: 'Sertifikat Std', symbol: 'u_sertf.' } // To match standard report syntax
    if (c.name.includes('Drift Std'))
      return { ...c, name: 'Drif Std', symbol: 'u_drift' }
    if (c.name.includes('Res Uut'))
      return { ...c, name: 'Resolusi Uut', symbol: 'u_resolusi uut' }
    if (c.name.includes('Res Std'))
      return { ...c, name: 'Resolusi Std', symbol: 'u_resolusi std' }
    return c
  })
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex gap-4 mb-1 text-[15px]">
          ...
          <div>
            {formatDec(globalUutAvg)} {formatUnit(unitUut)}
          </div>
        </div>
        {!isPyranometerSensor && (
          ...
          <tbody>
            {components.map((c, i) => (
              <tr key={i}>
                ...
                <td className="border border-black px-2 py-0.5">
                  {formatDec(c.cov_factor, 3)}
                </td>
                <td className="border border-black px-2 py-0.5">
                  {c.deg_freedom}
                </td>
                <td className="border border-black px-2 py-0.5">
                  {formatSci(c.std_uncertainty)}
                </td>
                <td className="border border-black px-2 py-0.5">
                  {c.sens_coeff}
                </td>
                ...
                <td className="border border-black px-2 py-0.5">
                  {formatSci(c.ci_ui_sq)}
                </td>
                <td className="border border-black px-2 py-0.5">
                  {formatSci(c.ci_ui_quad_vi)}
                </td>
              </tr>
            ))}
            ...
            <tr>
              ...
              <td className="border border-black px-1 py-0.5">
                {formatSci(result.sums.ci_ui_sq)}
              </td>
              <td className="border border-black px-1 py-0.5">
                {formatSci(result.sums.ci_ui_quad_vi)}
              </td>
            </tr>
            <tr>
              <td colSpan={9} className="border-0 border-r border-black"></td>
              ...
              >
                {formatSci(result.comb_uncert_uc)}
              </td>
            </tr>
            <tr>
              <td colSpan={9} className="border-0 border-r border-black"></td>
              ...
              <td
                ...
                className="border border-black px-1 py-0.5 text-center"
              >
                {result.eff_deg_freedom_veff.toLocaleString('id-ID', {
            ...
            <tr>
              ...
              <td
                ...
                className="border border-black px-1 py-0.5 text-center"
              >
                {result.cov_factor_95.toLocaleString('id-ID', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </td>
