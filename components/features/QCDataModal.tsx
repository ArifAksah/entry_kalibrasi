...
interface QCDataModalProps {
  ...
  onCalculateSaved?: (
    updates: Array<{ sensorId: number | string; table: any[] }>,
  ) => void | Promise<void>
  /**
...
const QCDataModal: React.FC<QCDataModalProps> = ({
  ...
  const isWindDirectionRow = (row: RawDataRow): boolean => {
    const sensor = row.sensor_id_uut
      ? sensors.find((s: any) => s.id === row.sensor_id_uut)
      : null
    const canonicalName = sensor?.sensor_name_id
      ? instrumentNames.find((name) => name.id === sensor.sensor_name_id)?.name
      : null
    const resultEntry = resultEntries.find(
      (entry) => entry.sensorId === row.sensor_id_uut,
    )
    return isWindDirectionSensor(
      { ...sensor, sheet_name: row.sheet_name },
      canonicalName,
      resultEntry?.calibrationMethod,
    )
  }
   ...
   */
  const [correctionMap, setCorrectionMap] = useState<Map<string, number>>(
    new Map(),
  )
  const [correctionLoading, setCorrectionLoading] = useState(false)

  const normalizedData = React.useMemo(() => {
    if (!Array.isArray(data) || data.length === 0 || resultEntries.length === 0)
      return data
    ...
    data.forEach((row) => {
      const key =
        row.sheet_name && row.sheet_name.trim() !== ''
    ...
    const pairs = normalizedData
      ...
      .map((row) => ({
        ...
        standardCertificateId:
          row.standard_certificate_id ??
          resultEntries.find((entry) => entry.sensorId === row.sensor_id_uut)
            ?.standardCertificateId,
      }))
  ...
  useEffect(() => {
    ...
    if (
      sensorKeys.length > 0 &&
      (activeTab === 0 ||
        (activeTab === 'unknown' && !sensorKeys.includes('unknown')))
    ) {
      setActiveTab(
        sensorKeys[0] === 'unknown' ? 'unknown' : Number(sensorKeys[0]),
      )
    }
  ...
  useEffect(() => {
    ...
    const pairs = normalizedData
      ...
      .map((r) => ({
        reading: r.standard_data as number,
        sensorStdId: r.sensor_id_std!,
      }))
  ...
  const handleDownloadExcel = () => {
    ...
    const rows = currentData.map((row, index) => {
      const { stdCorrection, stdCorrected, uutCorrection, qc } =
        computeRowQC(row)
      ...
      return {
        ...
        ...(showCalculationAudit && audit
          ? {
              ...
              'Selisih Koreksi STD Aktif - Tersimpan':
                audit.storedCorrectionDifference ?? '',
    ...
    if (showCalculationAudit && auditComparison) {
      const summary = XLSX.utils.json_to_sheet([
        {
          Profil: 'Sistem',
          Jumlah: auditComparison.system.count,
          'Rata-rata': auditComparison.system.mean,
          'STDEV.S': auditComparison.system.standardDeviation,
        },
        {
          Profil: 'Excel Legacy',
          Jumlah: auditComparison.legacy.count,
          'Rata-rata': auditComparison.legacy.mean,
          'STDEV.S': auditComparison.legacy.standardDeviation,
        },
        {
          Profil: 'Selisih',
          Jumlah: auditComparison.difference.count,
          'Rata-rata': auditComparison.difference.mean,
          'STDEV.S': auditComparison.difference.standardDeviation,
        },
      ])
  ...
  const currentData = [...(groupedData[String(activeTab)] || [])].sort(
    compareRawDataRows,
  )
   ...
   */
  const getStdCorrection = (
    row: RawDataRow,
  ): {
    value: number
    hasData: boolean
    source: 'active' | 'stored' | 'missing' | 'pyranometer'
  } => {
    if (row.standard_data == null)
    ...
    if (!row.sensor_id_std)
      return { value: 0, hasData: false, source: 'missing' }
    const key = `${row.sensor_id_std}:${row.standard_data}`
    const standardCertificateId =
      row.standard_certificate_id ??
      resultEntries.find((entry) => entry.sensorId === row.sensor_id_uut)
        ?.standardCertificateId
    ...
    if (localCorrectionMap.has(localKey)) {
      return {
        value: localCorrectionMap.get(localKey)!,
        hasData: true,
        source: 'active',
      }
    }
    ...
    if (hasData)
      return { value: correctionMap.get(key)!, hasData: true, source: 'active' }
  ...
  const computeRowQC = (row: RawDataRow) => {
    ...
    if (isPyrano) {
      ...
      // ═══════════════════════════════════════════════════════════
      rawStdCorrected = hasStdValue
        ? (row.standard_data as number) + stdCorrection
        : null
      ...
      if (!unitUut && row.sensor_id_uut) {
        ...
        unitUut =
          uutSensor?.graduating_unit || uutSensor?.range_capacity_unit || ''
      }
      hasConversion = !!(
        unitStd &&
        unitUut &&
        needsConversion(unitStd, unitUut)
      )
      stdCorrectedInUutUnit =
        rawStdCorrected == null
          ? null
          : hasConversion
  ...
  const auditRows = currentData
    ...
    .filter(
      (
        ...
        row: RawDataRow
        audit: NonNullable<ReturnType<typeof computeAuditRow>>
      } => item.audit != null,
    )
  const auditComparison =
    auditRows.length > 0
      ? {
          system: calculateAuditStats(
            auditRows.map((item) => item.audit.systemCorrection),
          ),
          legacy: calculateAuditStats(
            auditRows.map((item) => item.audit.legacyCorrection),
          ),
          difference: calculateAuditStats(
            auditRows.map((item) => item.audit.correctionDifference),
          ),
          storedCorrectionMismatchCount: auditRows.filter(
            (item) =>
              item.audit.storedCorrectionDifference != null &&
              Math.abs(item.audit.storedCorrectionDifference) > 1e-12,
          ).length,
          storedFallbackCount: auditRows.filter(
            (item) => item.audit.correctionSource === 'stored',
          ).length,
  ...
  const stdSensorId =
    currentData.length > 0 ? currentData[0].sensor_id_std : null
    ...
    try {
      ...
      for (const key of sensorKeys) {
        ...
        const selectedCertificateId =
          groupData[0]?.standard_certificate_id ??
          resultEntries.find(
        ...
        const finalCorrectionPairs = isPyranometerSensor
          ...
          : rowsForCalc
              .map((row) => ({
                row,
                correction: computeRowQC(row).uutCorrection,
              }))
              .filter(
                (item): item is { row: RawDataRow; correction: number } =>
        ...
        if (isPyranometerSensor && uutSensor) {
          ...
          // ═══════════════════════════════════════════════════════════
          console.log(
            '[QCDataModal] PYRANOMETER DETECTED - Calculating CF-based correction',
          )
          ...
          const cfPerRow = rowsForCalc
            .map((row, idx) => {
              ...
              if (idx < 3)
                console.log(
                  `[QCDataModal] Row ${idx}: std=${std}, uut=${uut}, CF=${cf.toFixed(4)}%`,
                )
              return cf
            })
          ...
          if (cfPerRow.length > 0) {
            correctionAvg =
              cfPerRow.reduce((sum, c) => sum + c, 0) / cfPerRow.length
            console.log(
              '[QCDataModal] correctionAvg (pyranometer):',
              correctionAvg.toFixed(4) + '%',
            )
          } else {
          ...
          // ═══════════════════════════════════════════════════════════
          const corrections = finalCorrectionPairs.map(
            (item) => item.correction,
          )
          if (corrections.length > 0) {
            correctionAvg =
              corrections.reduce((sum, correction) => sum + correction, 0) /
              corrections.length
          }
        }
        ...
        if (isPyranometerSensor && uutSensor) {
          ...
          const result = calculateCalibrationResult({
            ...
            finalCorrections: finalCorrectionPairs.map(
              (item) => item.correction,
            ),
          })
        ...
        const newTable = [
          {
            title: isPyranometerSensor
              ? 'Hasil Kalibrasi Pyranometer / Pyranometer Calibration Result'
            ...
            rows: [
              {
                key: isPyranometerSensor
                  ? String(uutAvg)
                  : String(displayUutAvg), // Pyranometer: rata-rata UUT, Biasa: rata-rata UUT
                unit: isPyranometerSensor
                ...
                ...(uncertaintyDecision
                  ? {
                      uncertaintyMeta: {
                        ...
                        cmc_profile_id:
                          uncertaintyDecision.cmc?.profileId ?? null,
                        cmc_profile_code:
                          uncertaintyDecision.cmc?.profileCode ?? null,
                        cmc_version: uncertaintyDecision.cmc?.version ?? null,
                        cmc_value_native:
                          uncertaintyDecision.cmc?.cmcNative ?? null,
                        cmc_unit_native:
        ...
        updates.push({
          sensorId: key === 'unknown' ? 'unknown' : Number(key),
          table: newTable,
        })
      }
      ...
      if (calculationSnapshots.length > 0) {
        ...
        if (!snapshotResponse.ok) {
          ...
          throw new Error(
            payload.error || 'Gagal menyimpan snapshot kalkulasi raw data',
          )
        }
        const snapshotsById = new Map(
          uniqueSnapshots.map((snapshot) => [String(snapshot.id), snapshot]),
        )
        setData((current) =>
          current.map((row) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[60] backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-50">
              ...
              {currentData.length > 0 &&
                !isPyranometer(currentSensorForPyranometer) && (
                  <button
                    ...
                    className={`ml-1 px-2 py-1 rounded text-xs font-semibold transition-colors ${
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-600 p-2 hover:bg-gray-200 rounded-full"
          >
            <svg
              className="w-6 h-6"
              fill="none"
            ...
            <>
                  ...
                  {sensorKeys.map((key) => {
                    ...
                    return (
                      ...
                      >
                        {tabLabel}
                        <span
                          className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'}`}
                        >
                          {groupedData[key].length}
                        </span>
                        {key !== 'unknown' && qcLimits[key] !== undefined && (
                          <span
                            className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono ${limit ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
                          >
              ...
              <div className="flex-1 overflow-hidden flex flex-col p-6">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full">
                      ...
                      <div className="p-4 border-b border-amber-200 bg-amber-50 shrink-0">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <div>
                            <div className="text-xs font-bold text-amber-900 uppercase">
                              Audit Perbandingan Kalkulasi
                            </div>
                            <div className="text-[11px] text-amber-700 mt-0.5">
                              Formula resmi Sistem mengikuti workbook. Selisih
                              harus nol bila input dan koreksi STD identik.
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                          {[
                            ...
                            {
                              label: 'Excel Legacy',
                              stats: auditComparison.legacy,
                            },
                            {
                              label: 'Selisih',
                              stats: auditComparison.difference,
                            },
                          ].map(({ label, stats }) => (
                            <div
                              key={label}
                            ...
                            >
                              ...
                              <div className="text-xs text-gray-700 mt-1">
                                ...
                                <b className="font-mono">
                                  {stats.mean.toFixed(9)}
                                </b>
                              </div>
                              <div className="text-xs text-gray-700">
                                STDEV.S:{' '}
                                <b className="font-mono">
                                  {stats.standardDeviation.toFixed(9)}
                                </b>
                              </div>
                            </div>
                          ))}
                          <div className="bg-white rounded border border-amber-200 p-2">
                            ...
                            <div className="text-xs text-gray-700 mt-1">
                              Aktif vs tersimpan berbeda:
                            </div>
                            <div
                              className={`text-sm font-bold ${auditComparison.storedCorrectionMismatchCount > 0 ? 'text-red-600' : 'text-green-700'}`}
                            >
                              {auditComparison.storedCorrectionMismatchCount}{' '}
                              baris
                            </div>
                            <div
                              className={`text-[10px] mt-1 ${auditComparison.storedFallbackCount > 0 ? 'text-amber-700' : 'text-gray-500'}`}
                            >
                              Fallback koreksi tersimpan:{' '}
                              {auditComparison.storedFallbackCount} baris
                            </div>
                            {auditComparison.storedFallbackReadings.length >
                              0 && (
                              <div className="text-[10px] text-amber-700 mt-1 break-words">
                                STD:{' '}
                                {auditComparison.storedFallbackReadings.join(
                                  ', ',
                                )}
                                {auditComparison.storedFallbackCount >
                                auditComparison.storedFallbackReadings.length
                                  ? ', ...'
                                  : ''}
                              </div>
                  ...
                  {/* Stats row */}
                  <div
                    className={`grid gap-3 p-4 border-b border-gray-100 shrink-0 ${isPyranometer(currentSensorForPyranometer) ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-2 md:grid-cols-5'}`}
                  >
                    ...
                    <div
                      className={`p-3 rounded-lg border ${activeSensorLimit ? 'bg-green-50 border-green-100' : 'bg-yellow-50 border-yellow-100'}`}
                    >
                      <div
                        className={`text-[10px] font-semibold uppercase ${activeSensorLimit ? 'text-green-600' : 'text-yellow-600'}`}
                      >
                        Batas WMO (Master QC)
                      </div>
                      <div
                        className={`text-sm font-bold ${activeSensorLimit ? 'text-green-900' : 'text-yellow-800'}`}
                      >
                        {qcLimitsLoading ? (
                            ...
                            <div className="mt-0.5 space-y-0.5">
                              <div className="flex items-baseline justify-between gap-2">
                                ...
                                <span className="text-sm font-bold tabular-nums">
                                  {correctionStats.mean.toFixed(4)}
                                </span>
                              </div>
                              <div className="flex items-baseline justify-between gap-2">
                                <span className="text-[10px] font-normal text-blue-600">
                    ...
                    {isPyranometer(currentSensorForPyranometer) &&
                      (() => {
                        const stdValues = currentData
                          .filter((r) => r.standard_data != null)
                          .map((r) => r.standard_data as number)
                        const uutValues = currentData
                          .filter((r) => r.uut_data != null)
                          .map((r) => r.uut_data as number)
                        ...
                        return (
                          <div className="bg-gradient-to-r from-emerald-50 to-orange-50 p-2 rounded-lg border border-gray-200">
                            <div className="text-[9px] font-semibold text-gray-600 uppercase mb-1 text-center">
                              Statistik Kalibrasi
                            </div>
                  ...
                  <div className="flex-1 overflow-auto">
                    <table className="min-w-full divide-y divide-gray-200 relative">
                      <thead className="bg-[#1e377c] text-white sticky top-0 z-10">
                        <tr>
                          <th className="px-4 py-3 text-left text-xs font-semibold uppercase w-10">
                            No
                          </th>
                          ...
                          {showCalculationAudit &&
                            !isPyranometer(currentSensorForPyranometer) && (
                              <>
                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase bg-amber-900/30">
                                  Sistem
                                </th>
                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase bg-amber-900/30">
                                  Excel Legacy
                        ...
                        {currentData.length > 0 ? (
                          currentData.map((row, index) => {
                            ...
                            return (
                              <tr
                                key={`${row.id}-${index}`}
                                className={`${isFail ? 'bg-pink-50 hover:bg-pink-100' : 'hover:bg-gray-50'} transition-colors`}
                              >
                                ...
                                {showCalculationAudit && !isPyrano && (
                                  <>
                                    <td className="px-4 py-2 text-xs font-mono bg-amber-50/40">
                                      {audit
                                        ? audit.systemCorrection.toFixed(9)
                                        : '-'}
                                    </td>
                                    <td className="px-4 py-2 text-xs font-mono bg-amber-50/40">
                                      {audit
                                        ? audit.legacyCorrection.toFixed(9)
                                        : '-'}
                                    </td>
                                    <td
                                      className={`px-4 py-2 text-xs font-mono bg-amber-50/40 ${audit && Math.abs(audit.correctionDifference) > 1e-12 ? 'text-amber-800 font-bold' : 'text-gray-500'}`}
                                    >
                                      {audit
                                        ? audit.correctionDifference.toExponential(
                                            6,
                                          )
                                        : '-'}
                                    </td>
                                ...
                                {!isPyrano && (
                                  <>
                                    <td
                                      className={`px-4 py-2 text-sm font-bold ${isFail ? 'text-red-600' : 'text-green-600'}`}
                                    >
                                      {uutCorrection != null ? (
                                        <span className="inline-flex items-center gap-1.5">
                                          <span>
                                            {uutCorrection > 0 ? '+' : ''}
                                            {uutCorrection.toFixed(4)}
                                          </span>
        ...
        <div className="bg-white border-t border-gray-200 p-4 flex justify-between items-center shrink-0">
          <div className="text-xs text-gray-500 max-w-2xl">
            <span className="font-semibold text-gray-700">Alur:</span> Std
            Reading →{' '}
            <code className="bg-gray-100 px-1 rounded">hitung_koreksi()</code>{' '}
            (interpolasi DB) → Std Terkoreksi → Koreksi UUT = Std Terkoreksi −
            UUT Reading. Batas dari <b>Master QC</b> berdasarkan jenis sensor.
            {!activeSensorLimit && (
              <span className="ml-1 text-yellow-600">
                ⚠ Sensor belum ada di Master QC.
              </span>
            )}
          </div>
          <div className="flex items-center gap-3">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-amber-50 text-amber-800 border border-amber-200">
                  ...
                  <span>
                    ...
                    {certificateStatus === 'sent'
                      ? 'dikirim ke Verifikator'
                      : 'diverifikasi'}{' '}
                    — tabel tidak bisa diubah lagi.
                  </span>
                </div>
              )}
                ...
                >
                  {correctionLoading ? (
                    ...
                    <>
                      ...
                      <span className="text-[10px] bg-green-400/30 px-1.5 py-0.5 rounded-full font-normal">
                        Klik lagi utk update
                      </span>
                    </>
                  ) : (
                    <>
            ...
            <button
              onClick={onClose}
              className="px-6 py-2 bg-gray-800 text-white text-sm font-medium rounded-lg hover:bg-gray-900 transition-all"
            >
              Tutup
            </button>
          </div>
