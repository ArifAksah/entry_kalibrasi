...
import { checkWMOLimit, normalizeSensorType } from '../../lib/wmo-limits';
...
const QCDataModal: React.FC<QCDataModalProps> = ({
    ...
    // Determine Logic for CURRENT Active Tab
    const activeSensorInfo = React.useMemo(() => {
        if (activeTab === 'unknown' || activeTab === 0) {
            // Fallback to Instrument Name if sensor is unknown
            const inst = instruments.find(i => i.id === certificateInstrumentId);
            return {
                name: 'Unknown Sensor',
                type: inst ? normalizeSensorType(inst.name, inst.type || '') : 'Temperature'
            };
        }
        ...
        if (sensor) {
            return {
                ...
                type: normalizeSensorType(sensor.name, sensor.type || '')
            };
        }

        return { name: `Sensor #${activeTab}`, type: 'Temperature' };
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden">
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 border-b border-gray-100 bg-white shadow-sm shrink-0">
                                        <div className="bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                                            ...
                                            <div className="text-sm font-bold text-indigo-900 truncate" title={activeSensorInfo.name}>{activeSensorInfo.name}</div>
                                        </div>
                                        <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                                            <div className="text-xs text-blue-600 font-semibold uppercase">Type / Limit</div>
                                            <div className="text-sm font-bold text-blue-900">{activeSensorInfo.type}</div>
                                        </div>
                                        ...
                                        <table className="min-w-full divide-y divide-gray-200 relative">
                                            <thead className="bg-[#1e377c] text-white sticky top-0 z-10 shadow-sm">
                                                <tr>
                                                    ...
                                                    <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">WMO Limit</th>
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200">
                                                {currentData.length > 0 ? (
                                                    currentData.map((row, index) => {
                                                        const qc = checkWMOLimit(
                                                            activeSensorInfo.type as any,
                                                            row.uut_data,
                                                            row.standard_data
                                                        );
                ...
                <div className="bg-white border-t border-gray-200 p-4 flex justify-between items-center shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                    <div className="text-xs text-gray-500 max-w-xl">
                        <span className="font-semibold text-gray-700">Note:</span> WMO Limits are applied based on the detected type of the <b>active sensor tab</b>.
                        Pink rows indicate correction values that exceed the allowable limit for that sensor type.
                    </div>
                    <button
                        onClick={onClose}
