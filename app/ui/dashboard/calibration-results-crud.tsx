...
const CalibrationResultsCRUD: React.FC = () => {
  ...
  >([])
  const [tableResultFields, setTableResultFields] = useState<
    Array<{ key: string; value: string }>
  >([])
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data)) {
            ...
            data.forEach((n: Note) => {
              map[n.id] =
                `ID: ${n.id} - ${n.traceable_to_si_through || 'No description'}`
            })
            setNotesMap(map)
          } else {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteCalibrationResult(id)
    } catch {}
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <Breadcrumb
          items={[{ label: 'Calibration', href: '#' }, { label: 'Results' }]}
        />
        <button
          onClick={() => openModal()}
          ...
          {calibrationResults.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                {new Date(item.calibration_date_start).toLocaleDateString()}
              </td>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                {new Date(item.calibration_date_end).toLocaleDateString()}
              </td>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate">
                {item.calibration_place}
              </td>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                {item.sensor
                  ? sensorMap[item.sensor] || `Sensor ID: ${item.sensor}`
                  : 'None'}
              </td>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                {item.notes
                  ? notesMap[item.notes] || `Notes ID: ${item.notes}`
                  : 'None'}
              </td>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        calibration_date_start: e.target.value,
                      })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, calibration_date_end: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, calibration_place: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        notes: e.target.value ? parseInt(e.target.value) : null,
                      })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    ...
                    {notes.map((note) => (
                      <option key={note.id} value={note.id}>
                        ID: {note.id} -{' '}
                        {note.traceable_to_si_through || 'No description'}
                      </option>
                    ))}
                  </select>
                </div>
                <div className="sm:col-span-2">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-3">
                    {environmentFields.length === 0 ? (
                      <p className="text-sm text-gray-500 text-center py-4">
                        No environment fields added. Click "Add Field" to start.
                      </p>
                    ) : (
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Add key-value pairs for environment data. Example: U.95 =
                    0.4, koreksi = 0.2C
                  </p>
                </div>
                <div className="sm:col-span-2">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-3">
                    {tableResultFields.length === 0 ? (
                      <p className="text-sm text-gray-500 text-center py-4">
                        No table result fields added. Click "Add Field" to
                        start.
                      </p>
                    ) : (
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Add key-value pairs for table result data. Example:
                    measurement_1 = 25.5, measurement_2 = 26.1
                  </p>
                </div>
