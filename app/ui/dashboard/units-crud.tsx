...
export default function UnitsCRUD() {
  ...
  const insertSnippet = (snippet: string) => {
    ...
    const newVal =
      formUnitName.slice(0, start) + snippet + formUnitName.slice(end)
    ...
    setTimeout(() => {
      const cursorPos =
        start +
        snippet.length -
        (snippet.endsWith('{}') ? 1 : snippet.endsWith('{}{}') ? 3 : 0)
      input.focus()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editing
        ? { id: editing.id, unit: formUnitName }
        : { unit: formUnitName }
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {units.length === 0 ? (
                ...
                units.map((item, index) => (
                  ...
                  >
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 w-12">
                      {' '}
                      {index + 1}
                    </td>
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6 space-y-5">
                ...
                <div className="flex items-center justify-between p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-600">
                        Gunakan formula matematika/fisika
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${latexMode ? 'translate-x-6' : 'translate-x-1'}`}
                    />
                  </button>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    id="unit"
                    required
                    className={`block w-full border rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 sm:text-sm transition-colors font-mono ${
                      latexMode
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                      {[
                        ...
                        <div key={src} className="flex items-center gap-1.5">
                          <code className="text-[10px] bg-white border border-blue-200 px-1 py-0.5 rounded text-blue-800 font-mono">
                            {src}
                          </code>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-6">
            ...
            <p className="text-sm text-gray-600 mb-6">
              ...
              {confirmDelete.unit}&quot;? Data yang sudah dihapus tidak bisa
              dipulihkan.
            </p>
            <div className="flex justify-end space-x-2">
              <button
