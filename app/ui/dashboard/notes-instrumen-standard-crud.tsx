...
const NotesInstrumenStandardCRUD: React.FC = () => {
  ...
  const [form, setForm] = useState<NotesInstrumenStandardInsert>({
    notes: null,
    instrumen_standard: null,
  })
  ...
  const openModal = (item?: NotesInstrumenStandard) => {
    if (item) {
      ...
      setForm({
        notes: item.notes,
        instrumen_standard: item.instrumen_standard,
      })
    } else {
      setEditing(null)
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <Breadcrumb
          items={[
            { label: 'Sensors', href: '#' },
            { label: 'Notes Instrumen Standard' },
          ]}
        />
        <button
          onClick={() => openModal()}
        ...
        <Table headers={['Notes', 'Instrumen Standard (Sensor)', 'Actions']}>
          {items.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                {item.instrumen_standard ?? '-'}
              </td>
              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium space-x-2">
                {' '}
                <EditButton
                  onClick={() => openModal(item)}
                  title="Edit Notes Instrumen Standard"
                />
                <DeleteButton
                  onClick={() => handleDelete(item.id)}
                  title="Delete Notes Instrumen Standard"
                />
              </td>
            </tr>
          ))}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        instrumen_standard: e.target.value
