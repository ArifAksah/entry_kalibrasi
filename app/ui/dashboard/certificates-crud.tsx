...
import {
  finalizeCertificateUncertaintyWithMaster,
...
const SearchableDropdown = ({
  ...
  const filteredOptions = options.filter((option) => {
    ...
    return (
      ...
      (option.station_id &&
        String(option.station_id).toLowerCase().includes(searchLower)) ||
      ...
      >
        {selectedOption ? (
          <span className="text-gray-900">
            {renderOptionName
              ? renderOptionName(selectedOption.name)
              : selectedOption.name}
...
const CertificatesCRUD: React.FC = () => {
  ...
  const fetchSignedPdf = async (item: Certificate, download = false) => {
    ...
    if (!contentType.toLowerCase().includes('application/pdf')) {
      ...
      throw new Error(
        `Response download bukan PDF yang valid.${errorText ? ` ${errorText.slice(0, 160)}` : ''}`,
      )
    }
  ...
  const devResetForResign = async (item: Certificate) => {
    ...
    try {
      ...
      const signerInfo = payload?.signer
        ...
        : ''
      showSuccess(
        (payload?.message || 'Sertifikat di-reset untuk TTE ulang') +
          signerInfo,
      )
      refetchCertificates()
    } catch (err) {
      ...
      showError(
        err instanceof Error ? `Reset gagal: ${err.message}` : 'Reset gagal',
      )
    }
    setActionDropdownOpenId(null)
  }
  ...
  const [actionDropdownOpenId, setActionDropdownOpenId] = useState<
    number | null
  >(null)
  const [isRejectionNotesModalOpen, setIsRejectionNotesModalOpen] =
    useState(false)
  const [selectedRejectionCertificate, setSelectedRejectionCertificate] =
    useState<Certificate | null>(null)

  // Filter & search state untuk daftar sertifikat
  const [searchQuery, setSearchQuery] = useState('')
  const [filterStatus, setFilterStatus] = useState<
    'all' | 'draft' | 'sent' | 'verified' | 'rejected' | 'completed'
  >('all')
  ...
  const [personel, setPersonel] = useState<
    Array<{ id: string; name: string; nip?: string; role?: string }>
  >([])
  ...
  const [uncertaintyModalCertificate, setUncertaintyModalCertificate] =
    useState<Certificate | null>(null)
  ...
  const [lhksCertificate, setLhksCertificate] = useState<Certificate | null>(
    null,
  )
  const [lhksRawData, setLhksRawData] = useState<any[]>([])
  const [lhksLoading, setLhksLoading] = useState(false)

  const getAnyResultSessionId = (rawResults: unknown): string | null => {
    const entries = resultsToLegacyView(rawResults)
    return (
      entries.map((r: any) => r.session_id).find((sid: any) => !!sid) ?? null
    )
  }
  ...
  const getFullSensorsForInstrument = (
    instrument: number | string | null | undefined,
  ): any[] => {
  ...
  const signatoryOptions = (
    ...
    if (selectedId && !options.some((o) => o.id === String(selectedId))) {
      ...
      if (selected)
        options.unshift(
          toOption({ ...selected, nip: selected.nip || undefined }),
        )
    }
    return options
  }
  ...
  const handlePreviewLHKS = async (cert: Certificate) => {
    ...
    if (!sessionId) {
      setLhksLoading(false)
      showError('Data mentah tidak tersedia untuk sertifikat ini.')
      return
    }
    try {
      ...
      showError('Gagal mengambil data mentah untuk preview LHKS')
    } finally {
      setLhksLoading(false)
    }
  }
  ...
  const [useStationAddressForPlace, setUseStationAddressForPlace] =
    useState(false)
  ...
  const resolveStationAddress = React.useCallback(
    (stationId?: number | null, explicitAddress?: string | null) => {
      const address =
        explicitAddress ??
        (stationId
  ...
  const removeResult = (idx: number) => {
    if (results.length > 1 || rawData.length > 0) {
      if (
        confirm('Hapus input sensor ini? Data yang sudah diisi akan hilang.')
      ) {
  ...
  useEffect(() => {
    ...
    const stationAddress = resolveStationAddress(
      form.station,
    ...
    setResults((prev) =>
      prev.map((r, i) =>
        i === resultIdx
          ? {
              ...r,
              images: (r.images || []).filter((_, idx) => idx !== imageIdx),
            }
          : r,
      ),
    )
  ...
  const getSelectedStationType = () => {
    const selectedStation = form.station
      ? stations.find((s) => s.id === form.station)
      : undefined
  ...
  const applySensorToResult = (idx: number, sensorId: number | null) => {
    const sensor = sensors.find((s: any) => s.id === sensorId) as unknown as
      Sensor | undefined
    updateResult(idx, {
  ...
  const [standardPickerIndex, setStandardPickerIndex] = useState<number | null>(
    null,
  )
  ...
  const getWorkbookSheetXmlPaths = (workbook: any): Record<string, string> => {
    const workbookRelsXml = decodeWorkbookFileText(
      findWorkbookFile(workbook, 'xl/_rels/workbook.xml.rels'),
    )
    ...
    workbookSheets.forEach((sheet: any, index: number) => {
      ...
      if (target) {
        pathsByName[sheetName] = `xl/${String(target)
          .replace(/^\/?xl\//i, '')
          .replace(/^\.\.\//, '')}`.replace(/\\/g, '/')
        return
      }
  ...
  const extractRowsFromSheetXml = (
    sheetXml: string,
    sharedStrings: any[] = [],
  ): any[][] => {
    const sheetDataMatch = sheetXml.match(
      /<(?:\w+:)?sheetData[^>]*>([\s\S]*?)<\/(?:\w+:)?sheetData>/i,
    )
    ...
    const readCellValue = (attrsSource: string, content: string): any => {
      ...
      const inlineStringMatch = content.match(
        /<(?:\w+:)?t[^>]*>([\s\S]*?)<\/(?:\w+:)?t>/i,
      )
      ...
      if (cellType === 's') {
        ...
        return Number.isFinite(stringIndex)
          ? (sharedStrings?.[stringIndex]?.t ??
              sharedStrings?.[stringIndex]?.r ??
              decodedValue)
          : decodedValue
      }
      ...
      return decodedValue !== '' && Number.isFinite(numericValue)
        ? numericValue
        : decodedValue
    }
  ...
  const handleAutoGenerate = async (sectionIndex: number) => {
    ...
    try {
      ...
      if (!sessionId) {
        showError(
          'Data QC tidak tersedia. Pastikan sensor ini sudah memiliki Data Raw tersimpan (session_id).',
        )
      ...
      const activeUutSensor = sensors.find(
        (s) => s.id === currentResult.sensorId,
      )
      ...
      const canonicalSensorName = activeUutSensor?.sensor_name_id
        ? instrumentNames.find(
            (name) => name.id === activeUutSensor.sensor_name_id,
          )?.name
        : null
      ...
      // ═══════════════════════════════════════════════════════════════
      const pyranometerSensorData: PyranometerSensorData | null =
        activeUutSensor
          ? {
      ...
      if (isPyranometerSensor && activeUutSensor) {
        ...
        const rawUnit =
          currentData[0]?.unit_uut ||
          activeUutSensor.graduating_unit ||
          activeUutSensor.range_capacity_unit ||
          'W/m²'
  ...
  const handleExcelUpload = async (
    ...
    try {
      ...
      if (newRows.length > 0) {
        setTableDraft((prev) => {
          ...
          newDraft[sectionIndex] = {
            ...newDraft[sectionIndex],
            rows: [
              ...newDraft[sectionIndex].rows.filter(
                (r) => r.key || r.unit || r.value,
              ),
              ...newRows,
            ],
          }
      ...
      showSuccess(
        `Berhasil mengimport data. Klik tombol mata untuk melihat raw data.`,
      )
  ...
  const handleRawDataUpload = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0]
    if (!file) return
    ...
    try {
      const data = await file.arrayBuffer()
      const workbook: any = read(data, {
        raw: true,
        cellDates: true,
        bookFiles: true,
      })
      const sheetXmlPaths = getWorkbookSheetXmlPaths(workbook)
      const sharedStrings = Array.isArray(workbook.Strings)
        ? workbook.Strings
        : []
      ...
      workbook.SheetNames.forEach((name: string) => {
        ...
        const sheetXml = decodeWorkbookFileText(
          sheetXmlPath ? findWorkbookFile(workbook, sheetXmlPath) : null,
        )
        const xmlRows = sheetXml
          ? extractRowsFromSheetXml(sheetXml, sharedStrings)
          : []
        ...
        if (cellAddresses.length > 0) {
          ...
          if (
            !ref ||
            utils.decode_range(computedRef).e.r > utils.decode_range(ref).e.r
          ) {
            ref = computedRef
          }

          console.log(
            `[raw-upload] Sheet "${name}" ref metadata=${refFromMetadata || 'none'} computed=${computedRef} cells=${cellAddresses.length}`,
          )
        }
        ...
        if (xmlRows.length > 0) {
          ...
          console.log(
            `[raw-upload] Sheet "${name}" xmlRows=${xmlRows.length} xmlPath=${sheetXmlPath || 'unknown'}`,
          )
        } else if (ref) {
          ...
          for (let R = range.s.r; R <= range.e.r; ++R) {
            ...
            if (
              row.some((val) => val !== undefined && val !== null && val !== '')
            ) {
              jsonData.push(row)
            }
          }
        ...
        if (jsonData.length > 0) {
          ...
          for (let i = 0; i < Math.min(jsonData.length, 10); i++) {
            const headers = (jsonData[i] as any[]).map((h) =>
              String(h).toLowerCase().trim(),
            )
            const hasTimestamp = headers.some(
              (h) =>
            ...
            const hasStandard = headers.some(
              (h) =>
                h.includes('standar') ||
                h.includes('standard') ||
                h.includes('ref') ||
          ...
          if (detectedHeaderIndex !== -1) {
            ...
            console.log(
              `[raw-upload] Sheet "${name}" headerRow=${detectedHeaderIndex + 1}`,
              detectedHeaders,
            )
            sheetsData.push({ name, data: normalizedData })
          } else {
            const previewHeaders = (jsonData[0] as any[]).map((h) =>
              String(h).toLowerCase().trim(),
            )
            const missing = []
            if (
            ...
            if (
              ...
              missing.push('UUT/Reading')
            invalidSheets.push(
              `${name} (Missing: ${missing.join(', ') || 'Header tidak ditemukan pada 10 baris awal'})`,
            )
          }
        }
      })
      ...
      if (invalidSheets.length > 0) {
        showError(
          `Beberapa sheet ditolak karena format header tidak sesuai: \n${invalidSheets.join('\n')}`,
        )
      }
      ...
      if (sheetsData.length > 0) {
        if (isDirty) {
          shouldProceed = confirm(
            `Terdeteksi ${sheetsData.length} sheet valid. Apakah Anda ingin mereset input sensor dan menyesuaikan dengan jumlah sheet?`,
          )
        }
        ...
        if (shouldProceed) {
          ...
          const newResults: ResultItem[] = sheetsData.map((sheet) => {
            ...
            const headers = (sheet.data[0] as any[]).map((h) =>
              String(h).toLowerCase().trim(),
            )
            const tempIdx = headers.findIndex(
              (h) => h.includes('suhu') || h.includes('temp'),
            )
            const humidIdx = headers.findIndex(
              (h) =>
                h.includes('kelembaban') ||
                h.includes('humidity') ||
                h.includes('rh'),
            )
        ...
        showSuccess(
          `Berhasil load ${sheetsData.length} sheet dari ${file.name}.`,
        )
      }
    } catch (error) {
    ...
    const fetchData = async () => {
      try {
        ...
        const fetchAllStations = async () => {
          ...
          if (!first?.ok)
            return { data: [], total: 0, pageSize: 100, totalPages: 1 }
          const firstJson = await first.json()
          const firstData = Array.isArray(firstJson)
            ? firstJson
            : (firstJson?.data ?? [])
          const totalPages = (
            Array.isArray(firstJson) ? 1 : (firstJson?.totalPages ?? 1)
          ) as number
          if (totalPages <= 1)
            return {
              data: firstData,
              total: (firstJson?.total ?? firstData.length) as number,
              pageSize: 100,
              totalPages,
            }
          ...
          const rest = await Promise.all(
            restPages.map((p) =>
              fetchJsonSafe(`${baseUrl}&page=${p}`, { data: [] }),
            ),
          )
          ...
          return {
            data: [...firstData, ...restData],
            total: (firstJson?.total ??
              firstData.length + restData.length) as number,
            pageSize: 100,
            totalPages,
          }
        }
        ...
        const fetchAllSensors = async () => {
          ...
          const rest = await Promise.all(
            restPages.map((p) =>
              fetchJsonSafe(`/api/sensors?page=${p}&pageSize=100`, {
                data: [],
              }),
            ),
          )
        ...
        const stationsRaw = Array.isArray(stationsAll)
          ? stationsAll
          : ((stationsAll as any)?.data ?? [])
        // Remove duplicate stations by id
        const stationsMap = new Map()
        stationsRaw.forEach((s: any) => {
          if (s?.id != null) stationsMap.set(s.id, s)
        })
        setStations(Array.from(stationsMap.values()))
        const instrumentsRaw = Array.isArray(instrumentsAll)
          ? instrumentsAll
          : ((instrumentsAll as any)?.data ?? [])
        const instrumentsMap = new Map()
        instrumentsRaw.forEach((i: any) => {
          if (i?.id != null) instrumentsMap.set(i.id, i)
        })
        setInstruments(Array.from(instrumentsMap.values()))
        const sensorsRaw = Array.isArray(sensorsAll)
          ? sensorsAll
          : ((sensorsAll as any)?.data ?? [])
        const sensorsMap = new Map()
        sensorsRaw.forEach((s: any) => {
          if (s?.id != null) sensorsMap.set(s.id, s)
        })
  ...
  */
  ...
  useEffect(() => {
    ...
    const nm = nameId
      ? (instrumentNames.find((n) => n.id === nameId) as any)
      : null
  ...
  const allowedCertificates = certificates
    ...
    .filter((item) => {
      ...
      const instrumentName =
        instruments.find((inst) => inst.id === item.instrument)?.name || ''
      const stationName =
        stations.find((st) => st.id === item.station)?.name || ''
      const haystack = [
        item.no_certificate,
        item.no_order,
  ...
  const currentCertificates = allowedCertificates.slice(
    indexOfFirstItem,
    indexOfLastItem,
  )
  ...
  const openModal = (item?: Certificate) => {
    if (item) {
      ...
      // Debugging log
      console.log(
        '[openModal] Opening edit for certificate:',
        item.no_certificate,
        {
          ...
          resultsPreview: Array.isArray((item as any)?.results?.sensors)
            ? (item as any).results.sensors.length
            : 0,
        },
      )
      setForm({
        ...
        station_address:
          (item as any).station_address ??
          (item.station
            ? (stations.find((s) => s.id === item.station)?.address ?? null)
            : null),
      ...
      const originalSensors = (() => {
        try {
          ...
          return Array.isArray(p?.sensors)
            ? p.sensors
            : Array.isArray(p)
              ? p
              : []
      ...
      const resolveStandardInstrumentId = (rawId: unknown): number | null => {
        ...
        const instrumentMatch = instruments.find(
          (inst: any) => inst.id === numericId,
        )
        if (instrumentMatch) return numericId

        // Legacy fallback: value actually points to sensor.id
        const sensorMatch = sensors.find(
          (sensor: any) => sensor.id === numericId,
        )
      ...
      const enrichedResults = savedResults.map((r: any, idx: number) => {
        ...
        if (orig?.links) {
          ...
          let matchedCert = std?.certificate_id
            ? (standardCerts.find(
                (c: any) => Number(c.id) === Number(std.certificate_id),
              ) ?? null)
            : null
          if (!matchedCert && std?.certificate_no) {
            matchedCert =
              standardCerts.find(
                (c: any) =>
                  String(c.no_certificate || '').trim() ===
                    String(std.certificate_no).trim() &&
                  (std?.sensor_id
                    ? Number(c.sensor_id) === Number(std.sensor_id)
                    : true),
              ) ??
              standardCerts.find(
                (c: any) =>
          ...
          if (!matchedCert && std?.sensor_id) {
            matchedCert =
              standardCerts.find(
                (c: any) => Number(c.sensor_id) === Number(std.sensor_id),
              ) ?? null
          }

          // 2) Resolve instrument ID: prefer stored value, else derive from
          //    the matched certificate's sensor.
          let resolvedStandardInstrumentId = resolveStandardInstrumentId(
            std?.instrument_id ?? std?.sensor_id,
          )
          if (!resolvedStandardInstrumentId && matchedCert?.sensor_id) {
            resolvedStandardInstrumentId = resolveStandardInstrumentId(
              matchedCert.sensor_id,
            )
          }

          // 3) Certificate number: prefer stored value, else the matched row.
          const restoredCertNo =
            std?.certificate_no && String(std.certificate_no).trim() !== ''
              ? std.certificate_no
          ...
          return {
            ...
            standardCertificateId:
              matchedCert?.id ?? r?.standardCertificateId ?? null,
            standardCertificateNumber:
              restoredCertNo ?? r?.standardCertificateNumber ?? null,
          }
        }
        ...
        return {
          ...
          standardInstrumentId:
            orig?.standardInstrumentId ?? r.standardInstrumentId ?? null,
          standardCertificateId:
            orig?.standardCertificateId ?? r.standardCertificateId ?? null,
          standardCertificateNumber:
            orig?.standardCertificateNumber ??
            r.standardCertificateNumber ??
            null,
      ...
      const sessionId =
        savedResults.map((r: any) => r.session_id).find((sid: any) => !!sid) ??
        (item as any).session_id ??
        null

      console.log(
        '[edit-open] certificate session ids:',
          ...
          .then((data) => {
            if (data.data && Array.isArray(data.data) && data.data.length > 0) {
              ...
              data.data.forEach((row: any) => {
                const sheetKey =
                  row.sheet_name && String(row.sheet_name).trim() !== ''
              ...
              const reconstructedSheets = Object.keys(grouped).map((key) => {
                ...
                const matchedSensor = sensorId
                  ? sensors.find((s: any) => s.id === sensorId)
                  : null
                const rowsForKey = data.data.filter((d: any) => {
                  const candidateKey =
                    d.sheet_name && String(d.sheet_name).trim() !== ''
                ...
                if (
                  storedSheetName &&
                  !/^\d+$/.test(String(storedSheetName).trim())
                ) {
                  sensorLabel = storedSheetName
                } else if (!matchedSensor) {
                  sensorLabel =
                    sensorId == null ? 'Unknown' : `Sensor ${sensorId}`
                } else {
                  const fromLookup = matchedSensor.sensor_name_id
                    ? instrumentNames.find(
                        (n: any) => n.id === matchedSensor.sensor_name_id,
                      )?.name
                    : undefined
                  if (fromLookup) {
                    ...
                    matchedSensor.name &&
                    !/^\d+$/.test(String(matchedSensor.name).trim())
                  ) {
              ...
              console.log(
                ...
                reconstructedSheets.map((sheet) => ({
                  name: sheet.name,
                  rowsIncludingHeader: Array.isArray(sheet.data)
                    ? sheet.data.length
                    : 0,
                  dataRows: Array.isArray(sheet.data)
                    ? Math.max(sheet.data.length - 1, 0)
                    : 0,
              ...
              setRawDataFilename(
                `Data Session ${sessionId} (${reconstructedSheets.length} sensor)`,
              )
            } else {
              setRawData([])
              setRawDataFilename(null)
      ...
      setForm({
        no_certificate:
          'Pilih Instrumen UUT & isi No. Identifikasi untuk preview…',
        no_order: '...',
        no_identification: '',
        authorized_by: null,
        verifikator_1: null as any,
      ...
      setResults([
        {
          ...
          table: [
            {
              title: '',
              rows: [{ key: '', unit: '', value: '', extraValues: [] }],
            },
          ], // Initialize with extraValues
          images: [],
          notesForm: createDefaultNotesForm(),
        },
      ])
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      ...
      fetch(`/api/certificates/generate-number?${qs}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editing && !(form as any).instrument_code) {
      showError(
        'Kode Alat belum tersedia. Pilih Instrumen UUT yang Nama Instrumen-nya sudah punya "Kode Instrumen" di menu Master Instrumen.',
      )
      return
    }
    ...
    try {
      ...
      if (form.instrument) {
        const selectedInstrument = instruments.find(
          (i) => i.id === form.instrument,
        )
        if (
          selectedInstrument &&
          !selectedInstrument.name_alias &&
          !selectedInstrument.instrument_names_id
        ) {
          // Generate name from manufacturer + type + serial
          const generatedName =
            `${selectedInstrument.manufacturer || 'Unknown'} ${selectedInstrument.type || 'Instrument'} ${selectedInstrument.serial_number || ''}`.trim()
      ...
      const saveSessionAndRawData = async () => {
        try {
          const existingSessionId = editing
            ? getAnyResultSessionId(editing.results)
            : null
          ...
          if (!sessionRes.ok) {
            ...
            const errMsg =
              ...
              'Unknown error'
            throw new Error(
              `Gagal membuat sesi kalibrasi: ${errMsg} (Status: ${sessionRes.status})`,
            )
          }

          {
            ...
            if (rawData.length > 0) {
              ...
              const missingUUT = rawData.some(
                (_, idx) => !results[idx]?.sensorId,
              )
              if (missingUUT) {
                throw new Error(
                  'Mohon pilih Sensor UUT untuk setiap sheet data mentah sebelum menyimpan.',
                )
              }
              ...
              const rawDataPayload = {
                ...
                data: rawData.map((sheet, idx) => {
                  ...
                  const uutSensor = uutSensorId
                    ? sensors.find((s: any) => s.id === uutSensorId)
                    : null
                  const uutUnit =
                    results[idx]?.unitUut ||
              ...
              console.log(
                'DEBUG: Sending Raw Data Payload:',
              ...
              if (!rawRes.ok) {
                ...
                throw new Error(
                  `Gagal menyimpan data mentah: ${errMsg} (Status: ${rawRes.status})`,
                )
              }

              const rawResData = await rawRes.json()
              console.log(
                'Raw Data saved linked to session:',
      ...
      if (editing) {
        ...
        console.log(
          '[certificate-update] final results session ids:',
        ...
        // Update Certificate
        await updateCertificate(editing.id, {
          ...payload,
      ...
      const errorMessage =
        e instanceof Error
          ? e.message
          : 'Terjadi kesalahan saat menyimpan certificate'
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const errorMessage =
        e instanceof Error
          ? e.message
          : 'Terjadi kesalahan saat menghapus certificate'
      showError(errorMessage)
    } finally {
      setIsDeleting(null)
    }
  ...
  const handleCompleteRepair = async (certificate: Certificate) => {
    ...
    if (result.success) {
      ...
      showError(
        'Gagal menyelesaikan perbaikan: ' + (result.error || 'Unknown error'),
      )
    }
  }
  ...
  const handleResetVerification = async (certificate: Certificate) => {
    if (
      !confirm(
        'Reset verifikasi untuk sertifikat ini? Ini akan menghapus semua verifikasi yang ada.',
      )
    )
      return
    ...
    if (result.success) {
      ...
      showError(
        'Gagal mereset verifikasi: ' + (result.error || 'Unknown error'),
      )
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <Spinner size="lg" tone="blue" className="border-[#1e377c]" />
      </div>
    )
  }
    ...
    <div className="space-y-4 max-w-full overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <label
                htmlFor="certificate-filter-status"
            ...
            <div className="flex items-center gap-2">
              <label
                htmlFor="certificate-filter-station"
                className="text-xs font-semibold text-gray-600 whitespace-nowrap"
              >
                Stasiun
              </label>
        ...
        {(searchQuery || filterStatus !== 'all' || filterStation !== 'all') && (
          <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500">
            ...
            <span className="font-semibold text-gray-800">
              {allowedCertificates.length}
            </span>{' '}
            sertifikat hasil filter
          </div>
        )}
      </div>
                  ...
                  <td colSpan={5} className="px-4 py-12">
                    ...
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      filterStation !== 'all' ? (
                        ...
                        <>
                          <p className="text-sm font-semibold text-gray-900">
                            Belum ada sertifikat selesai
                          </p>
                          <p className="mt-1 text-xs text-gray-500 max-w-md">
                            Daftar menampilkan hanya sertifikat berstatus{' '}
                            <strong>Completed</strong> untuk stasiun yang
                            ditugaskan ke akun Anda. Belum ada sertifikat yang
                        ...
                        <>
                          <p className="text-sm font-semibold text-gray-900">
                            Tidak ada sertifikat yang relevan
                          </p>
                          <p className="mt-1 text-xs text-gray-500 max-w-md">
                            Belum ada sertifikat yang sesuai dengan akun Anda.
                            Sertifikat akan tampil di sini setelah Anda
                            ditugaskan sebagai pembuat, verifikator, atau
                            penandatangan.
                          </p>
                        </>
                      )}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm">
                    ...
                    <div className="flex flex-col space-y-1">
                      <div className="flex items-center space-x-1">
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full border ${
                            (item as any).verifikator_1_status === 'approved'
                              ? 'bg-green-50 text-green-700 border-green-200'
                              : (item as any).verifikator_1_status ===
                                  'rejected'
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full border ${
                            (item as any).verifikator_2_status === 'approved'
                              ? 'bg-green-50 text-green-700 border-green-200'
                              : (item as any).verifikator_2_status ===
                                  'rejected'
                                ? 'bg-red-50 text-red-700 border-red-200'
                                : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                          }`}
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full border ${
                            (item as any).verifikator_3_status === 'approved'
                              ? 'bg-green-50 text-green-700 border-green-200'
                              : (item as any).verifikator_3_status ===
                                  'rejected'
                                ? 'bg-red-50 text-red-700 border-red-200'
                                : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                          }`}
                        >
                          {(item as any).verifikator_3_status === 'pending' ||
                          !(item as any).verifikator_3_status
                            ? 'Belum di periksa'
                            : (item as any).verifikator_3_status}
                        </span>
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full border ${
                            (item as any).authorized_by_status === 'approved'
                              ? 'bg-green-50 text-green-700 border-green-200'
                              : (item as any).authorized_by_status ===
                                  'rejected'
                                ? 'bg-red-50 text-red-700 border-red-200'
                                : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                          }`}
                        >
                          {(item as any).authorized_by_status === 'pending' ||
                          !(item as any).authorized_by_status
                            ? 'Belum di periksa'
                            : (item as any).authorized_by_status}
                        </span>
                  ...
                  <td className="px-4 py-3 text-sm">
                    ...
                    <div className="flex flex-wrap items-center gap-1">
                      ...
                      {(item as any).calibration_kind && (
                        <span
                          className={`px-1.5 py-0.5 text-[10px] font-bold rounded border ${(item as any).calibration_kind === 'LC' ? 'bg-violet-50 text-violet-700 border-violet-200' : 'bg-sky-50 text-sky-700 border-sky-200'}`}
                        >
                          {(item as any).calibration_kind}
                        </span>
                      )}
                      {(item as any).results_frozen_at && (
                        <span
                          className="px-1.5 py-0.5 text-[10px] font-semibold rounded border bg-gray-100 text-gray-500 border-gray-300"
                          title={`Results dibekukan: ${new Date((item as any).results_frozen_at).toLocaleString('id-ID')}`}
                        >
                          🔒
                        </span>
                      )}
                  ...
                  <td className="px-4 py-3 text-sm font-medium">
                    ...
                    >
                      <button
                        onClick={() =>
                          setActionDropdownOpenId(
                            actionDropdownOpenId === item.id ? null : item.id,
                          )
                        }
                      ...
                      >
                        Aksi
                        <ChevronDownIcon
                          className={`w-3.5 h-3.5 transition-transform duration-200 ${actionDropdownOpenId === item.id ? 'rotate-180' : ''}`}
                        />
                      </button>
                        ...
                        role !== 'user_station' && (
                          <div className="absolute right-0 mt-2 w-48 rounded-xl shadow-xl bg-white ring-1 ring-black ring-opacity-5 z-[50] py-1 animate-in fade-in slide-in-from-top-2 duration-200 border border-gray-100 divide-y divide-gray-50">
                            ...
                            <div className="py-1">
                              ...
                              <button
                                onClick={() => {
                                  const sessionId = getAnyResultSessionId(
                                    item.results,
                                  )
                                  ...
                                  if (sessionId) {
                                    ...
                                    showError(
                                      'Data QC tidak tersedia. Pastikan sertifikat ini memiliki data mentah yang tersimpan (session_id).',
                                    )
                                  }
                                }}
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    const res = await fetch(
                                      `/api/raw-data?session_id=${sessionId}&lean=true`,
                                    )
                                    if (!res.ok)
                                    ...
                                    console.error(
                                      'Failed to fetch raw data for Uncertainty',
                                      e,
                                    )
                                    showError(
                                      'Gagal mengambil data mentah untuk perhitungan ketidakpastian',
                                    )
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/50">
            <div className="text-xs text-gray-600">
              ...
              <span className="font-semibold">
                {Math.min(indexOfLastItem, certificates.length)}
              </span>{' '}
              of <span className="font-semibold">{certificates.length}</span>{' '}
              entries
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <>
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            placeholder={
                              form.instrument
                                ? 'Kode alat tidak tersedia untuk instrumen ini'
                                : 'Pilih Instrumen UUT dulu…'
                            }
                            className="w-full px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-700 text-sm"
                          />
                          {form.instrument &&
                            !(form as any).instrument_code && (
                              <p className="text-[11px] text-red-500 italic">
                                Nama Instrumen terhubung ke instrumen ini belum
                                punya "Kode Instrumen". Minta admin hubungkan di
                    ...
                    <div className="space-y-1">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newBalaiId = e.target.value
                    ...
                    <div className="space-y-1">
                      ...
                      {!editing && (
                        <p className="text-[11px] text-gray-500 italic">
                          Nomor final akan ditetapkan otomatis saat disimpan
                    ...
                    {[
                      {
                        ...
                        value: form.no_identification,
                        onChange: (e: any) =>
                          setForm({
                            ...form,
                            no_identification: e.target.value,
                          }),
                        type: 'text',
                        required: true,
                      },
                    ...
                    <div className="space-y-1">
                      ...
                      <SearchableDropdown
                        ...
                        onChange={(value) =>
                          setForm({
                            ...form,
                            authorized_by: value as string | null,
                          })
                        }
                    ...
                    <div className="space-y-1">
                      ...
                      <SearchableDropdown
                        ...
                        onChange={(value) =>
                          setForm({
                            ...form,
                            verifikator_1: value as string | null,
                          } as any)
                        }
                        options={signatoryOptions(
                          ['verifikator'],
                          (form as any).verifikator_1 ?? null,
                        )}
                    ...
                    <div className="space-y-1">
                      ...
                      <SearchableDropdown
                        ...
                        onChange={(value) =>
                          setForm({
                            ...form,
                            verifikator_2: value as string | null,
                          } as any)
                        }
                        options={signatoryOptions(
                          ['verifikator'],
                          (form as any).verifikator_2 ?? null,
                        )}
                    ...
                    <div className="space-y-1">
                      ...
                      <SearchableDropdown
                        ...
                        onChange={(value) =>
                          setForm({
                            ...form,
                            verifikator_3: value as string | null,
                          } as any)
                        }
                        options={signatoryOptions(
                          ['verifikator'],
                          (form as any).verifikator_3 ?? null,
                        )}
                  ...
                  <div className="mb-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-amber-800">
                        Hasil kalibrasi telah dibekukan
                      </p>
                      <p className="text-xs text-amber-700 mt-0.5">
                        ...
                        . Bagian II dan III tidak dapat diedit. Untuk membuka
                        kembali, sertifikat harus dikembalikan ke status Draft
                        melalui alur penolakan verifikator.
                      </p>
                    </div>
                  </div>
                )}
                ...
                >
                          ...
                          startDate={
                            sessionDetails.start_date
                              ? (() => {
                                  ...
                                  if (isNaN(d.getTime()))
                                    return sessionDetails.start_date.split(
                                      'T',
                                    )[0]
                                  const pad = (n: number) =>
                          ...
                          endDate={
                            sessionDetails.end_date
                              ? (() => {
                                  ...
                                  if (isNaN(d.getTime()))
                                    return sessionDetails.end_date.split('T')[0]
                                  const pad = (n: number) =>
                                    n.toString().padStart(2, '0')
                                  return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`
                                })()
                        ...
                        <SearchableDropdown
                          ...
                          options={instruments
                            .filter((i) => {
                              ...
                              if (form.station) {
                                const stationId =
                                  i.station?.id || (i as any).station_id
                                if (String(stationId) !== String(form.station))
                                  return false
                              }

                              // Filter UUT Only (Exclude Standards)
                              // Check if any sensor is marked as standard
                              const isStandard = i.sensor?.some(
                                (s: any) => s.is_standard === true,
                              )
                              return !isStandard
                            })
                            .map((i) => {
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                      <div className="flex-1 space-y-3">
                        <div
                          className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${rawDataFilename ? 'border-green-300 bg-green-50/30' : 'border-blue-200 bg-blue-50/50 hover:bg-blue-50'}`}
                        >
                          <input
                            type="file"
                            className="hidden"
                          ...
                          >
                            ...
                            {isImporting ? (
                              ...
                              <>
                                <span className="text-sm font-semibold text-gray-700">
                                  Klik untuk upload Excel/CSV
                                </span>
                                <span className="text-xs text-gray-500 mt-1">
                                  Format: Multi-sheet (Sheet 1 = Sensor 1, dst)
                                </span>
                              </>
                            )}
                          </label>
                      ...
                      <div className="flex-1 bg-gray-50 rounded-lg p-4 border border-gray-200">
                        ...
                        <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                          {rawData.length === 0 ? (
                            <div className="text-xs text-center text-gray-400 py-4 italic">
                            ...
                            rawData.map((sheet, i) => {
                              ...
                              return (
                                <div
                                  key={i}
                                  className="p-2 bg-white border border-gray-200 rounded shadow-sm space-y-2"
                                >
                                  {/* Sheet header row */}
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs font-mono font-bold bg-gray-100 px-1.5 py-0.5 rounded text-gray-500">
                                      #{i + 1}
                                    </span>
                                    ...
                                    {results[i]?.unitUut &&
                                      ...
                                      results[i]?.unitUut !==
                                        ...
                                        <div className="text-[10px] text-orange-600 bg-orange-50 rounded px-1.5 py-0.5">
                                          ⚠️ Unit berbeda — STD (
                                          {results[i]?.unitStd}) akan dikonversi
                                          ke UUT ({results[i]?.unitUut}) sebelum
                                          hitung koreksi
                                        </div>
                                      )}
                                    {results[i]?.unitUut &&
                        ...
                        {rawData.length > 0 && (
                          ...
                          >
                            <span className="flex items-center justify-center gap-2">
                              <EyeIcon className="w-4 h-4" /> Lihat Preview Raw
                              Data
                            </span>
                          </button>
                        )}
                  ...
                  {results.map((result, resultIndex) => {
                    ...
                    return (
                      <div
                        key={resultIndex}
                        className="bg-gray-50/50 rounded-xl border border-gray-200 p-4 mb-6 relative"
                      >
                        {/* Loop Header */}
                            ...
                            <h4 className="text-sm font-bold text-gray-800">
                              ...
                              {rawData[resultIndex] && (
                                <span className="ml-2 px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-normal">
                                  Sheet: {rawData[resultIndex].name}
                                </span>
                              )}
                            </h4>
                          </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          {/* 1. Alat UUT */}
                          <div
                            className={`bg-white rounded-xl shadow-sm border border-gray-200 p-4 relative overflow-hidden group transition-all ${!form.instrument ? 'opacity-60 pointer-events-none grayscale' : 'hover:border-[#1e377c]/30'}`}
                          >
                              ...
                              <div className="space-y-1">
                                ...
                                <SearchableDropdown
                                  ...
                                  options={sensors
                                    .filter((s) =>
                                      form.instrument
                                        ? s.instrument_id === form.instrument
                                        : true,
                                    )
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <div className="space-y-1">
                                    ...
                                    <input
                                      type="text"
                                      value={
                                        result.sensorDetails?.manufacturer ||
                                        instrumentPreview.manufacturer ||
                                        ''
                                      }
                                      onChange={(e) =>
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input
                                      type="text"
                                      value={
                                        result.sensorDetails?.type ||
                                        instrumentPreview.type ||
                                        ''
                                      }
                                      onChange={(e) =>
                                        updateResult(resultIndex, {
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input
                                      type="text"
                                      value={
                                        result.sensorDetails?.serial_number ||
                                        instrumentPreview.serial ||
                                        ''
                                      }
                                      onChange={(e) =>
                                        updateResult(resultIndex, {
                            ...
                            <div className="space-y-4">
                                ...
                                <SearchableDropdown
                                  ...
                                  onChange={(val) => {
                                    ...
                                    const traceValue =
                                      ...
                                      (standardSensor as any)?.traceability ||
                                      ''
                                  ...
                                  options={(() => {
                                    ...
                                    const sorted = [...certsForThisCert].sort(
                                      (a, b) =>
                                        (hasRealData(b) ? 1 : 0) -
                                        (hasRealData(a) ? 1 : 0),
                                    )
                              ...
                              {(() => {
                                const selectedStandard = standardCerts.find(
                                  (c) => c.id === result.standardCertificateId,
                                )
                                return selectedStandard ? (
                                  <div className="grid grid-cols-2 gap-3 bg-green-50/50 p-3 rounded-lg border border-green-100">
                                    <div className="col-span-2 text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
                                      Detail Sertifikat & Sensor
                                    </div>
                                    {[
                                      {
                                        label: 'No. Sertifikat',
                                        val: selectedStandard.no_certificate,
                                      },
                                      {
                                        label: 'Tgl Kalibrasi',
                                        val: selectedStandard.calibration_date,
                                      },
                                      {
                                        label: 'Drift',
                                        val: selectedStandard.drift,
                                      },
                                      {
                                        label: 'U95',
                                        val: selectedStandard.u95_general,
                                      },
                                    ].map((f, i) => (
                                      <div key={i}>
                                        ...
                                        <div className="text-sm font-medium text-gray-800 text-ellipsis overflow-hidden">
                                          {f.val}
                                        </div>
                                      </div>
                                    ))}
                        ...
                        <div className="mt-6 border-t border-gray-200 pt-6 col-span-1 lg:col-span-2">
                          ...
                          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                                ...
                                {result.environment.length > 0 ? (
                                    ...
                                    return (
                                      <div key={envIdx} className="space-y-1">
                                        <div className="flex items-center justify-between">
                                          <label
                                            className={`block text-xs font-semibold ${isEnabled ? 'text-gray-600' : 'text-gray-400'}`}
                                          >
                                            {env.key}
                                          </label>
                                  ...
                                  <div className="col-span-2 text-center py-4 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                    <p className="text-xs text-gray-500 italic">
                                      Belum ada data kondisi lingkungan. Upload
                                      Data Mentah untuk auto-generate atau
                                      tambah manual.
                                    </p>
                                    <button
                                      type="button"
                            ...
                            {/* Notes & References */}
                            <div className="space-y-4">
                              <h5 className="text-xs font-bold text-gray-700 border-b pb-2 uppercase tracking-wide">
                                Catatan & Referensi
                              </h5>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                  ...
                                  />
                                  <datalist
                                    id={`traceability-options-${resultIndex}`}
                                  >
                                ...
                                <div className="space-y-1">
                                  ...
                                  />
                                  <datalist
                                    id={`ref-doc-options-${resultIndex}`}
                                  >
                                ...
                                <div className="space-y-1 md:col-span-2">
                                  <div className="flex items-center justify-between">
                                    <label
                                      className={`block text-xs font-semibold ${result.notesForm?.others_enabled ? 'text-gray-700' : 'text-gray-400'}`}
                                    >
                                      Lainnya / Komentar
                                    </label>
                                    <label className="relative inline-flex items-center cursor-pointer">
                                      <input
                                        ...
                                        className="sr-only peer"
                                        checked={Boolean(
                                          result.notesForm?.others_enabled,
                                        )}
                                        onChange={(e) =>
                                          updateResult(resultIndex, {
                                            notesForm: {
                                              ...result.notesForm,
                                              others_enabled: e.target.checked,
                                            },
                                          })
                                        }
                                      />
            ...
            <div className="max-h-[60vh] overflow-y-auto p-4 bg-gradient-to-br from-white to-gray-50/30">
              <div className="space-y-3">
                {envDraft.map((row, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-1 md:grid-cols-2 gap-3 p-3 border border-gray-200 rounded-lg bg-white"
                  >
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-gray-700">
                        Key
            ...
            <div className="flex justify-end space-x-2 p-4 border-t border-gray-200 bg-gray-50/50">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  updateResult(envEditIndex, {
                    environment: envDraft.filter((r) => r.key || r.value),
                  })
                ...
                {tableDraft.map((section, si) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="relative">
                        ...
                        <button
                          type="button"
                          disabled={isImporting}
                          className={`inline-flex items-center p-2 rounded-lg transition-all duration-200 border border-transparent ${
                            isImporting
                              ? 'bg-gray-100 text-gray-400 cursor-wait'
                    ...
                    <div className="space-y-2">
                      ...
                      <div
                        className={`grid grid-cols-1 md:grid-cols-${(section.headers || ['Parameter', 'Unit', 'Nilai']).length + 1} gap-2 p-2 border-b border-gray-100 bg-gray-100/50 rounded-t-lg`}
                      >
                        {(
                          section.headers || ['Parameter', 'Unit', 'Nilai']
                        ).map((header, hi) => (
                      ...
                      {section.rows.map((row, ri) => {
                        ...
                        return (
                          <div
                            key={ri}
                            className={`grid grid-cols-1 md:grid-cols-${(section.headers || ['Parameter', 'Unit', 'Nilai']).length + 1} gap-2 p-2 border border-gray-100 rounded bg-gray-50 relative`}
                          >
                            {(
                              section.headers || ['Parameter', 'Unit', 'Nilai']
                            ).map((header, colIdx) => (
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const interpolateCorrection = (
                                      ...
                                      const sorted = [...table].sort(
                                        (a, b) =>
                                          parseFloat(a.setpoint) -
                                          parseFloat(b.setpoint),
                                      )
                                      ...
                                      if (val < parseFloat(sorted[0].setpoint))
                                        return parseFloat(sorted[0].correction)
                                      if (
                                        val >
                                        parseFloat(
                                          sorted[sorted.length - 1].setpoint,
                                        )
                                      )
                                        return parseFloat(
                                          sorted[sorted.length - 1].correction,
                                        )
                                      return 0
                                    }
                                    ...
                                    if (
                                      (currentHeader.includes('standard') ||
                                        currentHeader.includes('standar')) &&
                                      selectedStandard?.correction_std
                                    ) {
                                      ...
                                      if (!isNaN(stdReading)) {
                                        ...
                                        const trueValue =
                                          stdReading + correction

                                        // Find "Correction" / "Koreksi" column index
                                        const headers = (
                                          section.headers || []
                                        ).map((h) => h.toLowerCase())
                                        const corrIdx = headers.findIndex(
                                          (h) =>
                                            h.includes('correction') ||
                                            h.includes('koreksi'),
                                        )
                                        const trueIdx = headers.findIndex(
                                          (h) =>
                                            h.includes('true') ||
                                            h.includes('benar') ||
                                            h.includes('sebenarnya'),
                                        )
                ...
                {getSelectedStationType() === 'geofisika' ? (
                  <div className="space-y-3">
                    <div className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                      ...
                      <div className="space-y-3">
                        {(results[tableEditIndex]?.images || []).map(
                          (image, imgIdx) => (
                            <div
                              key={imgIdx}
                            ...
                            >
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    removeImage(tableEditIndex, imgIdx)
                                  }
                                    ...
                                    <input
                                      ...
                                      onChange={async (e) => {
                                        ...
                                        try {
                                          ...
                                          formData.append(
                                            'folder',
                                            `certificate-${editing ? editing.id : 'new'}`,
                                          )
                                          const res = await fetch(
                        ...
                        {(!results[tableEditIndex]?.images ||
                          (results[tableEditIndex]?.images || []).length ===
                            0) && (
                    ...
                    .map((sec) => ({
                      ...
                      rows: sec.rows.filter(
                        (r) =>
                          ...
                          r.unit ||
                          r.value ||
                          (r.extraValues && r.extraValues.some((v) => v)),
                      ),
                    }))
            ...
            <div className="max-h-[60vh] overflow-y-auto p-4 bg-gradient-to-br from-white to-gray-50/30">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNoteDraft((prev) => ({
                        ...prev,
                        traceable_to_si_through: e.target.value,
                      }))
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#1e377c] text-sm"
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNoteDraft((prev) => ({
                        ...prev,
                        calibration_methode: e.target.value,
                      }))
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#1e377c] text-sm"
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <label
                      className={`block text-xs font-semibold ${noteDraft.others_enabled ? 'text-gray-700' : 'text-gray-400'}`}
                    >
                      Others
                    </label>
                    <label className="relative inline-flex items-center cursor-pointer">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setNoteDraft((prev) => ({
                                  ...prev,
                                  standardInstruments:
                                    prev.standardInstruments.filter(
                                      (_, idx) => idx !== i,
                                    ),
                                }))
                              }
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-2 backdrop-blur-sm">
          <div className="w-full max-w-5xl bg-white rounded-xl shadow-xl overflow-hidden border border-[#1e377c] max-h-[90vh] flex flex-col">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-xs">
                    {rawDataFilename || 'Data Import'} • {rawData.length} Sheets
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-auto p-4 bg-white font-mono text-xs">
              {rawData[rawPreviewSheetIndex] ? (
                <table className="w-full border-collapse border border-gray-200">
                  <tbody>
                    {(rawData[rawPreviewSheetIndex].data as any[][])
                      .slice(0, 100)
                      .map((row, ri) => (
                        <tr
                          key={ri}
                        ...
                        >
                          ...
                          {row.map((cell, ci) => (
                            <td
                              key={ci}
                              className="p-2 border-r border-gray-100 last:border-r-0 whitespace-nowrap"
                            >
                              {' '}
                    ...
                    {(rawData[rawPreviewSheetIndex].data as any[][]).length >
                      100 && (
                      <tr>
                        <td
                          colSpan={20}
            ...
            <div className="p-3 bg-gray-50 border-t border-gray-200 text-right">
              <button
                onClick={() => setShowRawDataModal(false)}
                className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded text-xs font-bold"
              >
                Tutup Preview
              </button>
            </div>
                ...
                <div className="h-64 overflow-y-auto border border-gray-200 rounded-lg">
                  {sensors
                    ...
                    .filter((s) => {
                      ...
                      return (
                        !q ||
                        `${(s as any).name || ''} ${(s as any).manufacturer || ''} ${(s as any).type || ''} ${(s as any).serial_number || ''}`
                          .toLowerCase()
                          .includes(q)
                      )
                    })
                    .map((s) => (
                      ...
                      >
                        <div className="font-medium text-gray-900 text-sm">
                          {(s as any).name || (s as any).type || 'Sensor'} —{' '}
                          {(s as any).type || ''}
                        </div>
                        <div className="text-xs text-gray-500 mt-0.5">
                          {(s as any).manufacturer || ''} • SN{' '}
                          {(s as any).serial_number || '-'}
                        </div>
                      </button>
                    ))}
                </div>
            ...
            <div className="bg-[#1e377c] p-4 flex justify-between items-center">
              <div>
                <h3 className="text-white font-bold text-lg">
                  Tabel Koreksi Standar
                </h3>
                <p className="text-blue-200 text-xs">
                  {viewingCorrectionStandard.no_certificate} |{' '}
                  {viewingCorrectionStandard.calibration_date}
                </p>
              </div>
              <button
            ...
            <div className="p-0 max-h-[60vh] overflow-y-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="px-4 py-2.5 border-b">Setpoint</th>
                    <th className="px-4 py-2.5 border-b">Correction</th>
                    <th className="px-4 py-2.5 border-b">U95</th>
                  </tr>
                </thead>
                  ...
                  {(() => {
                    ...
                    return rows.length > 0 ? (
                      ...
                      <tr>
                        <td
                          colSpan={3}
                          className="px-6 py-8 text-center text-gray-500 italic bg-gray-50/30"
                        >
                          {' '}
      ...
      {isRejectionNotesModalOpen && selectedRejectionCertificate && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            {(() => {
              const latestRejection = getLatestRejectionEntry(
                selectedRejectionCertificate,
              )
              const rejectionHistory = getSortedRejectionHistory(
                selectedRejectionCertificate,
              )
              const rejectedBy = latestRejection?.rejected_by
                ? personel.find((p) => p.id === latestRejection.rejected_by)
                    ?.name || latestRejection.rejected_by
                : null
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 mt-1">
                        {getVerificationLevelLabel(
                          latestRejection?.verification_level,
                        )}
                      </p>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2">
                      ...
                      <p className="text-gray-900 mt-1 whitespace-pre-wrap">
                        {latestRejection?.rejection_reason ||
                          (selectedRejectionCertificate as any)
                            .rejection_reason ||
                          'Catatan penolakan tidak tersedia.'}
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="space-y-4">
                        {rejectionHistory.map((entry: any, index: number) => {
                          const entryRejectedBy = entry?.rejected_by
                            ? personel.find((p) => p.id === entry.rejected_by)
                                ?.name || entry.rejected_by
                            : null

                          return (
                            <div
                              key={`${entry.rejection_timestamp || index}-${entry.verification_level || 0}`}
                              className="flex gap-3"
                            >
                              <div className="flex flex-col items-center pt-1">
                                <div
                                  className={`w-2.5 h-2.5 rounded-full ${index === 0 ? 'bg-red-600' : 'bg-red-300'}`}
                                ></div>
                              ...
                              <div className="flex-1 pb-1">
                                <div className="flex flex-wrap items-center gap-2 mb-1">
                                  ...
                                  <span className="px-2 py-0.5 text-[11px] font-medium rounded-full bg-red-50 text-red-700 border border-red-200">
                                    {entry?.rejection_category_label ||
      ...
      {qcModalCertificate && (
        <QCDataModal
          ...
          sessionId={
            getAnyResultSessionId(qcModalCertificate.results) ?? undefined
          }
          ...
          resultEntries={resultsToLegacyView(qcModalCertificate.results).map(
            (r: any) => ({
              sensorId: r.sensorId ?? r.sensor_id ?? null,
          ...
          onCalculateSaved={async (updates) => {
            ...
            updates.forEach((update) => {
              const targetIdx =
                update.sensorId === 'unknown'
                  ? 0
                  : results.findIndex(
                      (r: any) =>
                        r.sensorId === update.sensorId ||
                        r.sensor_id === update.sensorId,
                    )
            ...
            try {
              ...
              showSuccess(
                `✅ Tabel hasil berhasil di-generate & disimpan ke sertifikat ${qcModalCertificate.no_certificate}!`,
              )
            } catch (err) {
              console.error(err)
        ...
        (() => {
          ...
          return (
            <LHKSReport
              ...
              owner={
                stations.find((s) => s.id === lhksCertificate.station) || null
              }
              instrument={
      ...
      {uncertaintyModalCertificate && (
        <UncertaintyModal
          ...
          instrumentNames={instrumentNames}
          dataLoading={uncertaintyLoading}
        />
      )}
    </div>
  )
