...
import {
  VerifikatorInspectionResult,
  VerifikatorInspectionResultInsert,
} from '../../../lib/supabase'
...
const VerifikatorInspectionResultsCRUD: React.FC = () => {
  const { items, loading, error, addItem, updateItem, deleteItem } =
    useVerifikatorInspectionResults()
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editing, setEditing] = useState<VerifikatorInspectionResult | null>(
    null,
  )
  ...
  const [form, setForm] = useState<VerifikatorInspectionResultInsert>({
    result: 0,
    verified_by: '',
  })
            ...
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium space-x-2">
                ...
                <EditButton
                  ...
                  title="Edit Verifikator Inspection Result"
                />
                <DeleteButton
                  onClick={() => handleDelete(item.id)}
                  title="Delete Verifikator Inspection Result"
                />
              </td>
            </tr>
          ))}
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <select
                    value={form.result || 0}
                    onChange={(e) =>
                      setForm({ ...form, result: parseInt(e.target.value) })
                    }
