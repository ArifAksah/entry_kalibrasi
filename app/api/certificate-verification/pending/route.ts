  ...
  try {
    ...
    if (certificateIds.length) {
      try {
        const { data: v, error: verifError } = await supabaseAdmin
          ...
          .select(
            "id, certificate_id, verification_level, status, created_at, certificate_version",
          )
    ...
    const certificatesWithStatus =
      (certificates || []).map((cert) => {
        ...
        const verif1 = verifications?.find(
          (v) =>
            v.certificate_id === cert.id &&
            v.verification_level === 1 &&
            (v.certificate_version ?? 1) === certVersion,
        );
        const verif2 = verifications?.find(
          (v) =>
            v.certificate_id === cert.id &&
            v.verification_level === 2 &&
            (v.certificate_version ?? 1) === certVersion,
        );
        const verif3 = verifications?.find(
          (v) =>
            v.certificate_id === cert.id &&
