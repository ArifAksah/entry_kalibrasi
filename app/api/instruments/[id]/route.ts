...
function getDeleteInstrumentErrorMessage(error: any) {
  ...
  if (
    message.includes("calibration_session_uut_instrument_id_fkey") ||
    message.includes("calibration_reference_std_instrument_id_fkey")
  ) {
  ...
  if (
    message.includes("raw_data_sensor_id") ||
    message.includes("calibration_result_sensor_fkey")
  ) {
...
export async function DELETE(
  ...
  try {
    ...
    if (!Number.isFinite(instrumentId)) {
      return NextResponse.json(
        { error: "ID instrumen tidak valid" },
        { status: 400 },
      );
    }
    ...
    if (sensorFetchError) {
      return NextResponse.json(
        { error: sensorFetchError.message },
        { status: 500 },
      );
    }

    const sensorIds = (sensors || [])
      .map((sensor: any) => Number(sensor.id))
    ...
    if (junctionDeleteError) {
      return NextResponse.json(
        { error: getDeleteInstrumentErrorMessage(junctionDeleteError) },
        { status: 500 },
      );
    }
    ...
    if (sensorIds.length > 0) {
      ...
      if (certificateDeleteError) {
        return NextResponse.json(
          { error: getDeleteInstrumentErrorMessage(certificateDeleteError) },
          { status: 500 },
        );
      }
      ...
      if (sensorDeleteError) {
        return NextResponse.json(
          { error: getDeleteInstrumentErrorMessage(sensorDeleteError) },
          { status: 500 },
        );
      }
    }
    ...
    if (error)
      return NextResponse.json(
        { error: getDeleteInstrumentErrorMessage(error) },
        { status: 500 },
      );
