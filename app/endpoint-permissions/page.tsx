...
const EndpointPermissionsPage: React.FC = () => {
  ...
  const [epForm, setEpForm] = useState<Partial<Endpoint>>({
    resource: '',
    method: 'GET',
    path: '',
    description: '',
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!r1.ok)
          throw new Error(d1?.error || 'Failed to load endpoint catalog')
  ...
  const filteredEndpoints = useMemo(() => {
    ...
    return endpoints.filter((ep) => {
      ...
      return (
        ep.resource.toLowerCase().includes(debouncedSearch) ||
    ...
    setPerms((prev) => {
      ...
      for (const ep of eps) {
        const idx = next.findIndex(
          (p) => p.role === role && p.endpoint_id === ep.id,
        )
  ...
  const toggle = (role: Role, endpoint_id: string) => {
    setPerms((prev) => {
      ...
      const exists = prev.find(
        (p) => p.role === role && p.endpoint_id === endpoint_id,
      )
      if (exists)
        return prev.map((p) => (p === exists ? { ...p, allow: !p.allow } : p))
  ...
  const save = async () => {
    try {
      setSaving(true)
      const r = await fetch('/api/role-endpoint-permissions', {
        method: 'PUT',
  ...
  const saveEndpoint = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = {
        resource: epForm.resource,
        method: epForm.method,
        path: epForm.path,
        description: epForm.description || '',
        enabled: epForm.enabled !== false,
        id: (epForm as any).id,
      }
      const method = editing ? 'PUT' : 'POST'
      const r = await fetch('/api/endpoint-catalog', {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(editing ? [body] : body),
      })
      const d = await r.json()
  ...
  const removeEndpoint = async (id: string) => {
    ...
    const r = await fetch(`/api/endpoint-catalog?id=${id}`, {
      method: 'DELETE',
    })
          ...
          <div className="p-4 sm:p-6 mx-auto max-w-[1600px]">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={async () => {
                    const r = await fetch('/api/endpoint-catalog/scan', {
                      method: 'POST',
                    })
                    const d = await r.json()
                    if (!r.ok) {
                      ...
                      await refreshCatalog()
                      showSuccess(`Scanned ${d?.count ?? 0} endpoints`)
                    }
                  }}
                        ...
                        {(() => {
                          ...
                          return (
                            <input
                              ...
                              ref={(el) => {
                                if (el)
                                  el.indeterminate = state.some && !state.all
                              }}
                              onChange={(e) =>
                        ...
                        return (
                          <td key={r} className="px-4 py-2">
                            ...
                            <input
                              ...
                              ref={(el) => {
                                if (el)
                                  el.indeterminate = !state.all && state.some
                              }}
                              onChange={(e) =>
                                toggleResource(r, resource, e.target.checked)
                              }
                            />
                          </td>
                        )
          ...
          <div className="bg-white w-full max-w-xl rounded-lg shadow-lg p-6 mx-4">
            ...
            >
              <div>
                ...
                <input
                  value={epForm.resource as any}
                  onChange={(e) =>
                    setEpForm({ ...epForm, resource: e.target.value })
                  }
                  required
                  className="w-full px-3 py-2 border rounded"
                  placeholder="sensor / instrument / certificate / station"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <select
                  value={epForm.method as any}
                  onChange={(e) =>
                    setEpForm({ ...epForm, method: e.target.value })
                  }
                  required
                  className="w-full px-3 py-2 border rounded"
                >
              ...
              <div className="md:col-span-2">
                ...
                <input
                  value={epForm.path as any}
                  onChange={(e) =>
                    setEpForm({ ...epForm, path: e.target.value })
                  }
                  required
                  className="w-full px-3 py-2 border rounded"
                  placeholder="/api/sensors or /api/sensors/:id"
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <textarea
                  value={epForm.description as any}
                  onChange={(e) =>
                    setEpForm({ ...epForm, description: e.target.value })
                  }
                  rows={3}
                  className="w-full px-3 py-2 border rounded"
                />
              ...
              <div className="md:col-span-2 flex items-center justify-between">
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  <input
                    type="checkbox"
                    checked={epForm.enabled !== false}
                    onChange={(e) =>
                      setEpForm({ ...epForm, enabled: e.target.checked })
                    }
                  />{' '}
                  Enabled
                </label>
