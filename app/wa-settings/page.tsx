...
export default function WASettingsPage() {
  ...
  const fetchStatus = async () => {
    try {
      ...
      setError(
        'WA Service tidak dapat dihubungi. Pastikan service berjalan di port 3001.',
      )
      setStatus(null)
      setQrData(null)
    } finally {
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {loading && (
                <div className="bg-white rounded-xl shadow-sm border p-8 text-center">
                  <Spinner size="lg" tone="blue" className="mx-auto mb-3" />
                  <p className="text-gray-500">
                    Menghubungkan ke WA Service...
                  </p>
                </div>
              )}
              ...
              {!loading && status && (
                <>
                  {/* Connection Status */}
                  <div
                    className={`rounded-xl shadow-sm border p-6 mb-6 ${status.connected ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-3 h-3 rounded-full ${status.connected ? 'bg-green-500' : 'bg-yellow-500'} animate-pulse`}
                        />
                        <div>
                  ...
                  {!status.connected && qrData && (
                    <div className="bg-white rounded-xl shadow-sm border p-8 text-center">
                      ...
                      <p className="text-sm text-gray-500 mb-6">
                        Buka WhatsApp → Settings → Linked Devices → Link a
                        Device
                      </p>
                      ...
                      <p className="text-xs text-gray-400 mt-4">
                        QR code akan refresh otomatis setiap beberapa detik
                      </p>
                    </div>
                  )}
                  ...
                  {!status.connected && !qrData && (
                    <div className="bg-white rounded-xl shadow-sm border p-8 text-center">
                      <Spinner size="lg" tone="blue" className="mx-auto mb-3" />
                      <p className="text-gray-500">
                        Menunggu QR code dari WA Service...
                      </p>
                      <p className="text-xs text-gray-400 mt-2">
                        Pastikan WA Service berjalan
                      </p>
                    </div>
                  )}
                      ...
                      <div className="space-y-3 text-sm text-gray-600">
                        ...
                        <div className="flex justify-between py-2 border-b border-gray-100">
                          ...
                          <span className="text-gray-900">
                            Konfirmasi akun, Sertifikat terbit, Draft ke
                            verifikator
                          </span>
                        </div>
