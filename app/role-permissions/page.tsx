...
const RolePermissionsPage: React.FC = () => {
  ...
  const pageKeys = useMemo(
    () => new Set(pagedRows.map((r) => `${r.role}:${r.resource}`)),
    [pagedRows],
  )
  const pageStateFor = useCallback(
    (key: keyof Row) => {
  ...
  const toggleAllOnPage = useCallback(
    (key: keyof Row, value: boolean) => {
      setRows((prev) =>
        prev.map((r) =>
          pageKeys.has(`${r.role}:${r.resource}`) ? { ...r, [key]: value } : r,
        ),
      )
    },
    [pageKeys],
  ...
  const toggle = (role: Role, resource: Resource, key: keyof Row) => {
    setRows((prev) =>
      prev.map((r) =>
        r.role === role && r.resource === resource
          ? { ...r, [key]: !(r as any)[key] }
          : r,
      ),
    )
                    ...
                    >
                      ...
                      {actionDefs.map((a) => (
                        <td key={a.key} className="px-4 py-2">
                          ...
                          <input
                            type="checkbox"
                            checked={(row as any)[a.key]}
                            onChange={() =>
                              toggle(row.role, row.resource, a.key as keyof Row)
                            }
                          />
                        </td>
                      ))}
