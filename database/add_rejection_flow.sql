-- Add rejection flow functionality to certificate system
-- This script adds columns to handle rejection destinations and flow

-- Add rejection destination columns to certificate_verification table
ALTER TABLE certificate_verification ADD COLUMN IF NOT EXISTS rejection_destination VARCHAR(20) DEFAULT 'creator' CHECK (rejection_destination IN ('creator', 'verifikator_1'));
ALTER TABLE certificate_verification ADD COLUMN IF NOT EXISTS rejection_reason_detailed TEXT;
ALTER TABLE certificate_verification ADD COLUMN IF NOT EXISTS rejection_timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- Add rejection flow tracking to certificate table
ALTER TABLE certificate ADD COLUMN IF NOT EXISTS rejection_count INTEGER DEFAULT 0;
ALTER TABLE certificate ADD COLUMN IF NOT EXISTS last_rejection_by UUID REFERENCES personel(id);
ALTER TABLE certificate ADD COLUMN IF NOT EXISTS last_rejection_at TIMESTAMP WITH TIME ZONE;
ALTER TABLE certificate ADD COLUMN IF NOT EXISTS rejection_history JSONB DEFAULT '[]'::jsonb;

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_certificate_verification_rejection_destination ON certificate_verification(rejection_destination);
CREATE INDEX IF NOT EXISTS idx_certificate_verification_rejection_timestamp ON certificate_verification(rejection_timestamp);
CREATE INDEX IF NOT EXISTS idx_certificate_rejection_count ON certificate(rejection_count);
CREATE INDEX IF NOT EXISTS idx_certificate_last_rejection_by ON certificate(last_rejection_by);

-- Create a view for rejection flow tracking
...
-- Create function to handle rejection flow
...
-- Create function to get rejection options for verifikator 2
...
-- Test the functions
SELECT 'Rejection flow functions created successfully!' as message;
